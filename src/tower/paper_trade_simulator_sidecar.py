#!/usr/bin/env python3
"""
QSB Tower V1.3 — Paper Trade Simulator Sidecar V1
Read-only. No order paths. No execution unlocks.
"""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            if parsed.path == "/api/paper_trades/status":
                send(self, sim.status())
            elif parsed.path == "/api/paper_trades/build":
                send(self, sim.build(instruments))
            else:
    ...
    def do_POST(self):
        try:
            ...
            from tower.paper_trade_simulator import PaperTradeSimulator
            send(self, PaperTradeSimulator().build(instruments))
        except Exception as exc:
            send(self, {"ok": False, "error": str(exc)}, 500)


def main():
    print(f"Paper Trade Simulator sidecar running at http://{HOST}:{PORT}")
