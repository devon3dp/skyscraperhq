...
.train{filter:drop-shadow(0 0 12px currentColor) drop-shadow(0 0 4px #fff)}
...
function routePath(A,B){
  ...
  // HORIZONTAL into the target column first, then VERTICAL along it into the target.
  // Convergent hubs (spine, gene_pool) therefore share ONE vertical lane instead of a
  // fan of diagonals — this is what removes the central knot.
...
function routeMid(A,B){
  ...
  return {x:B.x, y:A.y};   // the corner of the L (horizontal-then-vertical)
}
...
function drawZones(d,R){
  ...
  const px=d.grid_pitch_x||210, y0=d.grid_y0||150, vh=d.grid_view_h||1000;
  const top=18, bot=vh-8;
  zb.forEach(z=>{
    const x0=z.x0-px*0.5, x1=z.x1+px*0.5, w=x1-x0;
...
function draw(d){
  ...
  d.lines.forEach(L=>{const A=ST[L.a],B=ST[L.b];if(!A||!B)return;
    ...
    if(mesh){
      ...
      if(act){ mkRail({stroke:(CC.route||"#40b4ff"),"stroke-width":5,opacity:.9});
        if(_z>=0.9)dirArrow(R,A,B,L); return; }
      if(showMesh) mkRail({stroke:"#2b3c52","stroke-width":1.2,opacity:.16,"stroke-dasharray":"4 7"},false);
      return; }
    if(act){
      // TRUNK EMPHASIS (item 3): primary trunk lines render visibly STRONGER than secondary.
      const w=L.trunk?8:5.5;
      mkRail({stroke:(CC[L.cat]||"#40b4ff"),"stroke-width":w,opacity:.96});
      ...
      mkRail({stroke:(CC[L.cat]||"#40b4ff"),"stroke-width":3,opacity:.30});
    } else {  // idle non-trunk categorised track: reduced idle-infra brightness (item 6)
      mkRail({stroke:"#33465f","stroke-width":1.3,opacity:.14,"stroke-dasharray":"3 6"});
    }});
  Object.entries(ST).forEach(([id,s])=>{
    ...
    if(s.floor){
      ...
      if(lit){  // lit halo so an active floor pops out of the dim elevation
        R.appendChild(el("circle",{cx:s.x,cy:s.y,r:fr+3,fill:"none",stroke:zc,"stroke-width":1.6,opacity:.7}));
      }
    ...
    if(online&&!s.offline&&s.has_traffic){
      R.appendChild(el("circle",{cx:s.x,cy:s.y,r:r+5,fill:"none",stroke:"#45f59b","stroke-width":2,opacity:.8,id:"on_"+id}));
    } else if(s.comms_online&&!s.offline){
      R.appendChild(el("circle",{cx:s.x,cy:s.y,r:r+5,fill:"none",stroke:"#3fbf8f","stroke-width":2,opacity:.7,"stroke-dasharray":"3 4",id:"on_"+id}));
    } else if(s.reachable){  // provider probed-reachable but not routing work
      R.appendChild(el("circle",{cx:s.x,cy:s.y,r:r+4,fill:"none",stroke:"#45f59b","stroke-width":1.6,opacity:.55,"stroke-dasharray":"2 3",id:"rc_"+id}));
    }
    ...
    if(s.ghost&&!s.has_traffic&&!s.offline){
      R.appendChild(el("circle",{cx:s.x,cy:s.y,r:r+4,fill:"none",stroke:"#c98a2a","stroke-width":1.6,opacity:.55,"stroke-dasharray":"2 4",id:"gh_"+id}));
    }
...
function spawnCarriage(tr,col){
  ...
  g.appendChild(el("rect",{x:A.x+2, y:A.y-8,width:17,height:16,rx:4,fill:col,
    "stroke":"#eafff5","stroke-width":1.4,opacity:isRet?.85:.96}));
  g.appendChild(el("circle",{cx:A.x+10,cy:A.y,r:3.4,fill:"#ffffff",opacity:.95}));
...
// ── PERF HUD (BEFORE-COPY, measurement only — identical to staging) ──
const _hud=document.createElement("div");
_hud.id="perfhud";
_hud.style.cssText="position:fixed;right:12px;bottom:12px;z-index:9999;font:12px/1.45 ui-monospace,monospace;"
...
function _hudFrame(t){_frames++;if(t-_lastT>=500){_fps=Math.round(_frames*1000/(t-_lastT));_frames=0;_lastT=t;
