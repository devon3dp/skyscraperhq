...
# V2B: next wiring approvals (each needs a separate Ross approval; nothing runs from here)
WIRING_APPROVALS = [
    {"title": "Approve Gmail connector wiring", "status": "NEEDS APPROVAL", "why": "Gmail PARTIAL — no live sync process; postfix only"},
    {"title": "Approve WhatsApp reply/send wiring", "status": "NEEDS APPROVAL", "why": "WhatsApp LIVE inbound; send disabled (DRAFT_ONLY)"},
    {"title": "Approve Telegram reply/send wiring", "status": "NEEDS APPROVAL", "why": "Telegram LIVE inbound; send disabled (DRAFT_ONLY)"},
    {"title": "Approve phone / Twilio wiring", "status": "NEEDS APPROVAL", "why": "Twilio bridge not running"},
    {"title": "Approve Galaxy bridge wiring", "status": "NEEDS APPROVAL", "why": "Galaxy bridge not running"},
    {"title": "Approve Pi kiosk setup", "status": "NEEDS APPROVAL", "why": "Pi 192.168.1.24 up (ping) but ports 22/80/8856 closed"},
    {"title": "Approve SSK / NAS mount", "status": "NEEDS APPROVAL", "why": "NAS 10.10.10.254 unreachable; not mounted"},
    {"title": "Approve Netlify public URL verification / deploy review", "status": "NEEDS APPROVAL", "why": "skyscraperhq.uk offline; no Netlify CLI to confirm/deploy"},
]
...
def probe_node(n: dict) -> dict:
    ...
    if kind == "not_ready":
        ...
        return out
    if kind == "file":
        # 2026-07-11 (Wren job job_1783777545, Ross-approved, R115 FIRST-WREN-JOB-FLOW-RECEPTIONIST-COUNCIL-LABEL):
        # judge freshness from the LIVE read-model snapshot (qsb_council_tasks_snapshot.json) via its own `ts`,
        # NOT the raw append-log mtime (idle-old when no NEW tasks). Raw log is fallback only. No task data edited.
        ...
        if snap and snap.get("ts"):
            try:
                st = datetime.fromisoformat(str(snap["ts"]).replace("Z", "+00:00"))
                age = int(datetime.now(timezone.utc).timestamp() - st.timestamp())
            except Exception:
                age = int(datetime.now(timezone.utc).timestamp() - SNAP.stat().st_mtime)
            tasks = snap.get("tasks", [])
            states = Counter(t.get("state", "?") for t in tasks) if isinstance(tasks, list) else Counter()
            out["status"] = "LIVE" if age < 3600 else "STALE"
            open_n = (states.get("open", 0) + states.get("routed_for_work", 0)
                      + states.get("needs_implementation", 0) + states.get("assigned", 0))
            out["detail"] = f"snapshot · {snap.get('total', len(tasks) if isinstance(tasks, list) else 0)} tasks · ~{open_n} open · updated {age}s ago"
        elif COUNCIL.exists():
            try:
                ...
                out["detail"] = f"raw_log · {len(lines)} rows · ~{open_n} open · updated {age}s ago"
            except Exception as e:
...
def network_status() -> dict:
    ...
    for d in net.get("devices", []):
        ...
        if d.get("fixed_status"):
            st = d["fixed_status"]  # verified override (e.g. Pi up-but-bare = PARTIAL)
        elif d.get("probe"):
...
def checklist_action(cid: str, action: str, note: str = "", by: str = "ross") -> dict:
    ...
    if action == "signoff":
        # Ross sign-off: records approval only; does NOT change status or execute anything.
        hit["signoff"] = by
        hit["notes"].append({"ts": now_iso(), "by": by, "text": "SIGNED OFF by Ross (no execution)"})
    elif action in m:
        hit["status"] = m[action]
        if action == "done":
            hit["checked_by"] = by
...
def commentary_lines() -> dict:
    """Truthful spoken lines built from real state. No fake activity."""
    st = build_state()
    live = st["live_count"]
    total = len(st["nodes"])
    stale = sum(1 for n in st["nodes"] if n["status"] == "STALE")
    notb = sum(1 for n in st["nodes"] if n["status"] == "NOT BUILT YET")
    na = sum(1 for a in APPROVAL_QUEUE if a["status"] == "NEEDS APPROVAL") + len(WIRING_APPROVALS)
    lines = [f"Receptionist is watching {total} systems. {live} are live."]
    for n in st["nodes"]:
        if n["status"] == "STALE":
            lines.append(f"{n['name']} is stale.")
        if n["status"] == "NOT BUILT YET":
            lines.append(f"{n['name']} is not built yet.")
        if n["key"] == "tp_pip" and n["status"] == "LIVE":
            lines.append("T P Pip is live on the Lenovo ThinkPad at dot 74, port 8871.")
        if n["key"] == "acer_cass" and n["status"] == "LIVE":
            lines.append("Acer Cass is live on the Acer at dot 41, port 8872.")
    try:
        for c in comms_status()["channels"]:
            if c["channel"] in ("WhatsApp", "Telegram") and c["status"] == "LIVE":
                lines.append(f"{c['channel']} is live, but sending is disabled.")
            if c["channel"] == "Gmail":
                lines.append("Gmail is partial. No live sync.")
            if c["channel"].startswith("Phone") and c["status"] != "LIVE":
                lines.append("Phone is not wired. Twilio is not live.")
            if c["channel"].startswith("Galaxy") and c["status"] != "LIVE":
                lines.append("The Galaxy bridge is not wired.")
    except Exception:
        pass
    w = load_websites()
    lines.append(f"{w.get('shops', 0)} shop folders were found. Public URLs still need verification.")
    lines.append(f"{na} approvals are waiting for Ross.")
    return {"ts": now_iso(), "lines": lines,
            "counts": {"live": live, "total": total, "stale": stale, "not_built": notb, "approvals": na}}
...
.ctrlwrap{max-width:1100px;margin:12px auto 0;padding:0 14px;display:flex;gap:12px;flex-wrap:wrap}
.avatarCard{flex:1;min-width:280px;background:var(--card);border:1px solid var(--line);border-radius:14px;padding:14px;display:flex;gap:14px;align-items:flex-start}
.rcAvatar{font-size:52px;line-height:1;width:70px;height:70px;display:flex;align-items:center;justify-content:center;border-radius:50%;background:#0e1c30;border:2px solid var(--cyan);flex:0 0 auto}
.rcAvatar.idle{animation:avBob 2.6s ease-in-out infinite}
.rcAvatar.speaking{animation:avSpeak .5s ease-in-out infinite;border-color:var(--live);box-shadow:0 0 22px var(--live)}
.rcAvatar.listening{border-color:var(--gold);box-shadow:0 0 22px var(--gold)}
@keyframes avBob{50%{transform:translateY(-5px)}}
@keyframes avSpeak{50%{transform:scale(1.08)}}
.ctrlPanel{flex:2;min-width:320px;background:var(--card);border:1px solid var(--line);border-radius:14px;padding:12px}
.ctrlHead{font-size:12px;color:var(--dim);text-transform:uppercase;letter-spacing:.06em;font-weight:800;margin-bottom:6px}
.ctrlRow{display:flex;gap:8px;flex-wrap:wrap}
.cbtn{flex:1 1 auto;min-width:140px;padding:13px 10px;font-size:13px}
"""
...
<!-- V2C: always-visible Receptionist Controls (avatar + voice + commentary + quick actions) -->
<div class="ctrlwrap">
 <div class="avatarCard">
  <div id="avatar" class="rcAvatar idle">🧑‍💼</div>
  <div style="flex:1;min-width:180px">
   <b style="font-size:16px">Receptionist · Iris</b> <span id="avState" class="pill OK">At desk</span>
   <div class="sub" style="margin-top:3px">🟢 issues <b id="avIssues">–</b> · ✅ approvals <b id="avAppr">–</b> · ⏱ last check <b id="avCheck">–</b></div>
   <div id="commentaryLine" style="margin-top:6px;color:var(--cyan);font-size:13px;min-height:18px">…</div>
   <div id="toneStatus" class="sub" style="margin-top:4px;font-size:11px;color:var(--stale)">🔊 audio on · commentary OFF</div>
   <div class="sub" style="font-size:10px;color:var(--wait)">idle avatar bob = COSMETIC · speaking glow only while TTS speaks · mic monitoring OFF by default</div>
  </div>
 </div>
 <div class="ctrlPanel">
  <div class="ctrlHead">🔊 Receptionist Voice</div>
  <div class="ctrlRow">
   <button class="btn cbtn" onclick="speakTest()">🧪 Test voice</button>
   <button class="btn cbtn" onclick="speakStatus()">🔊 Speak current status</button>
   <button class="btn cbtn" onclick="startCommentary()">▶️ Start commentary</button>
   <button class="btn cbtn" onclick="stopCommentary()">⏸️ Stop commentary</button>
   <button class="btn cbtn" id="muteBtn" onclick="toggleMute()">🔇 Mute</button>
   <button class="btn cbtn" onclick="toggleVoiceAdv()">🎛️ Voice settings</button>
  </div>
  <div class="ctrlRow" style="margin-top:6px;align-items:center;font-size:12px;color:var(--dim)">
   <button class="btn cbtn red" onclick="killAllAudio()">🔴 Kill all Receptionist audio</button>
   <label style="flex:1;min-width:170px"><input id="swCommDis" type="checkbox" onchange="vtoggle('commentaryDisabled')"> Disable live commentary completely</label>
   <label style="flex:1;min-width:150px"><input id="swSafe" type="checkbox" onchange="vtoggle('safeMode')"> Voice safe mode</label>
   <label style="flex:1;min-width:140px"><input id="swText" type="checkbox" onchange="vtoggle('textOnly')"> Text only mode</label>
  </div>
  <div class="sub" style="font-size:10px;color:var(--wait)">⚠ No microphone monitoring: "Start listening" is OFF by default; the browser only listens while you hold it on. Live commentary is OFF after every reload.</div>
  <div id="dupWarn" style="display:none;margin-top:8px;padding:8px;border-radius:8px;background:#3a0d0d;color:#ff6b6b;border:1px solid #ff6b6b;font-weight:700;font-size:12px">⚠ Another Receptionist tab may also be speaking — close duplicates or mute this tab.</div>
  <div id="voiceAdv" style="display:none;margin-top:8px;border-top:1px solid var(--line);padding-top:8px">
   <div class="ctrlRow" style="align-items:center">
    <select id="voiceSel" onchange="vset('voice',this.value)" style="flex:2 1 220px;padding:10px;background:#0b1322;color:var(--ink);border:1px solid var(--line);border-radius:8px"><option value="">(auto — best English)</option></select>
   </div>
   <div class="ctrlRow" style="align-items:center;font-size:12px;color:var(--dim)">
    <label style="flex:1;min-width:150px">Rate <b id="rateV">0.90</b><input id="rate" type="range" min="0.6" max="1.4" step="0.05" oninput="vset('rate',this.value)" style="width:100%"></label>
    ...
    <label style="flex:1;min-width:150px">Volume <b id="volV">1.00</b><input id="vol" type="range" min="0" max="1" step="0.05" oninput="vset('volume',this.value)" style="width:100%"></label>
   </div>
   <div class="ctrlRow" style="align-items:center;font-size:12px;color:var(--dim)">
    <label style="flex:1;min-width:160px">Commentary interval <select id="intv" onchange="vset('intervalMs',+this.value)" style="padding:6px;background:#0b1322;color:var(--ink);border:1px solid var(--line);border-radius:6px"><option value="6000">6s</option><option value="8000">8s</option><option value="10000">10s</option><option value="15000">15s</option></select></label>
    <label style="flex:1;min-width:190px"><input id="ovg" type="checkbox" onchange="vset('overlapGuard',this.checked)"> Prevent overlapping speech</label>
    <label style="flex:1;min-width:190px"><input id="pwh" type="checkbox" onchange="vset('pauseWhenHidden',this.checked)"> Pause speech when tab hidden</label>
   </div>
   <div class="ctrlRow">
    <button class="btn cbtn" onclick="speakIssue()">⚠️ Speak latest issue</button>
    <button class="btn cbtn" onclick="speakApprovals()">✅ Speak approvals</button>
    <button class="btn cbtn" onclick="speakReport()">📄 Speak latest report</button>
    <button class="btn cbtn" id="listenBtn" onclick="toggleListen()">🎤 Start listening</button>
    <button class="btn cbtn red" onclick="stopAll()">⛔ Stop all speech</button>
    <button class="btn cbtn" onclick="resetVoice()">↺ Reset</button>
    <button class="btn cbtn" onclick="voiceDebug()">🐞 Debug</button>
   </div>
   <div id="voiceDebug" class="sub" style="display:none;margin-top:8px;font-family:ui-monospace,monospace;font-size:11px;background:#0b1322;border:1px solid var(--line);border-radius:8px;padding:8px"></div>
   <div class="sub" style="font-size:11px;margin-top:8px;color:var(--wait)">Browser speech input (mic) is optional — may need Chromium permission.</div>
  </div>
  <div class="sub" style="font-size:11px;margin-top:8px;color:var(--stale)">🔉 Voice is local browser speech. If it sounds doubled, close duplicate tabs or press “Stop all speech” (in Voice settings).</div>
  <div class="ctrlHead" style="margin-top:8px">🛎️ Receptionist Controls</div>
  <div class="ctrlRow">
   <button class="btn cbtn" onclick="sysCheck()">🔄 Refresh status</button>
   <button class="btn cbtn" onclick="go('checklist')">☑️ Open checklist</button>
   <button class="btn cbtn" onclick="go('websites')">🛍️ Websites/Shops</button>
   <button class="btn cbtn" onclick="go('comms')">📡 Comms</button>
   <button class="btn cbtn" onclick="quickNote()">✎ Add note</button>
   <button class="btn cbtn" onclick="go('drafts')">📝 Draft task</button>
   <button class="btn cbtn gold" onclick="signoffLatest()">✍️ Sign off latest item</button>
   <button class="btn cbtn gold" onclick="attention()">🔔 Ross attention</button>
  </div>
 </div>
</div>

<div class="ctrlwrap" style="margin-top:12px">
 <div class="avatarCard" style="flex:1;display:block">
  <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
   <b style="font-size:15px">📜 Receptionist History — what Iris has been doing</b>
   <span style="margin-left:auto;display:flex;gap:6px;flex-wrap:wrap">
    <button class="btn cbtn" style="min-width:0;padding:8px 10px;font-size:12px" onclick="clearHistoryView()">Clear from view</button>
    <button class="btn cbtn" style="min-width:0;padding:8px 10px;font-size:12px" onclick="exportHistory()">Export snapshot</button>
    <button class="btn cbtn" style="min-width:0;padding:8px 10px;font-size:12px" onclick="addHistoryNote()">Add note</button>
    <button class="btn cbtn" style="min-width:0;padding:8px 10px;font-size:12px" onclick="readHistoryAloud()">🔊 Read aloud</button>
   </span>
  </div>
  <div id="histFilters" style="margin:6px 0"></div>
  <div class="sub" style="font-size:10px;color:var(--wait);margin-bottom:4px">Clear from view hides rows here but NEVER deletes the JSONL history (qsb_receptionist_activity.jsonl).</div>
  <div id="history" style="max-height:230px;overflow:auto;font-size:12px"></div>
 </div>
</div>

<div class="wrap">
 <section id="t_signoff" class="hide">
  <h2>Receptionist Signoff — Ross only (logs your acceptance; does NOT execute work)</h2>
  <div class="grid" id="signoff"></div>
 </section>
 <section id="t_desk">
 ...
 <section id="t_approvals" class="hide">
  ...
  <h2>Next wiring approvals (V2B — gaps to wire, each needs separate approval)</h2><div id="wiring"></div>
 </section>
...
const TABS=[['today','📋 Today'],['desk','🛎️ Desk'],['signoff','✔️ Signoff'],['floors','🏢 Floors'],['websites','🛍️ Websites'],['comms','📡 Comms'],['network','🌐 Network'],['checklist','☑️ Checklist'],['issues','⚠️ Issues'],['approvals','✅ Approvals'],['activity','📜 Activity'],['drafts','📝 Drafts'],['brief','🌅 Brief'],['inbox','📥 Inbox'],['handover','🔀 Handover'],['emergency','🚨 Emergency']];
const HFILTERS=['all','voice','approvals','checklist','websites','comms','network','reports','errors'];
let histFilter='all',histHidden=false;
function histMatch(e,f){if(f==='all')return true;const a=(e.action||'').toLowerCase(),i=(e.item||'').toLowerCase();const s=a+' '+i;
 if(f==='voice')return /comment|speak|voice|tts/.test(s);
 if(f==='approvals')return /approv|signoff|needs_approval/.test(s);
 if(f==='checklist')return /item_|checklist|done|clear|snooze|draft|seeded/.test(s);
 if(f==='websites')return /website|shop|netlify/.test(s);
 if(f==='comms')return /comm|gmail|whatsapp|telegram|reply/.test(s);
 if(f==='network')return /network|pi |nighthawk|ssk|galaxy|router/.test(s);
 if(f==='reports')return /report/.test(s);
 if(f==='errors')return /err|fail|refus|blocked/.test(s);return true;}
async function loadHistory(){const f=$('#histFilters');if(f)f.innerHTML=HFILTERS.map(x=>`<span class="pill ${x===histFilter?'NEEDS':'LOW'}" style="cursor:pointer;margin-right:4px" onclick="histFilter='${x}';loadHistory()">${x}</span>`).join('');
 const h=$('#history');if(!h)return;if(histHidden){h.innerHTML='<div class="li">(cleared from view — history preserved in JSONL)</div>';return;}
 const d=await j('/api/activity');const evs=(d.events||[]).filter(e=>histMatch(e,histFilter)).slice(0,25);window._hist=evs;
 h.innerHTML=evs.map(e=>`<div class="li" style="padding:6px 9px"><span style="color:var(--dim)">${(e.ts||'').replace('T',' ').slice(0,19)}</span> · <b>${e.action}</b> · ${e.actor}${e.item?(' · '+e.item):''}${e.result?(' → '+e.result):''}</div>`).join('')||'<div class="li">no history yet</div>';}
function clearHistoryView(){histHidden=!histHidden;loadHistory();toast(histHidden?'history hidden from view (JSONL kept)':'history shown');}
function exportHistory(){const data=JSON.stringify(window._hist||[],null,1);const b=new Blob([data],{type:'application/json'});const u=URL.createObjectURL(b);const a=document.createElement('a');a.href=u;a.download='receptionist_history_snapshot.json';a.click();URL.revokeObjectURL(u);toast('history snapshot exported');}
async function addHistoryNote(){const t=prompt('history note:');if(t){await post('/api/note',{text:t});toast('note added');loadHistory();}}
function readHistoryAloud(){const evs=(window._hist||[]).slice(0,5);if(!evs.length)return toast('no history');speak('Recent activity. '+evs.map(e=>e.action.replace(/_/g,' ')+(e.item?(', '+e.item):'')).join('. '),{interrupt:true});}
const SIGNFIELDS=['audio accepted','voice still faulty','avatar accepted','buttons accepted','history accepted','websites visible','comms visible','floor directory accepted','not ready'];
function loadSignoff(){const s=$('#signoff');if(!s)return;s.innerHTML=SIGNFIELDS.map(f=>`<div class="node"><b>${f}</b><div class="row"><button class="btn" style="flex:1;padding:9px" onclick="signoffMark('${f}','yes')">✓ Yes</button><button class="btn" style="flex:1;padding:9px" onclick="signoffMark('${f}','no')">✗ No</button></div></div>`).join('');}
async function signoffMark(field,value){await post('/api/signoff',{field:field,value:value});toast('signoff: '+field+' = '+value);speak('Noted. '+field+', marked '+value+'.',{interrupt:true});loadHistory();}
...
async function loadApprovals(){const d=await j('/api/approval_queue');$('#appr').innerHTML=d.queue.map(a=>`<div class="li">${labelPill(a.status)}<b>${a.title}</b><br><span style="color:var(--dim)">why: ${a.why} · risk: ${a.risk}</span><br><span style="color:var(--dim)">touches: ${a.touches} · not allowed: ${a.not_allowed}</span><br><span class="v">suggested: ${a.suggested}</span></div>`).join('');
 const w=await j('/api/wiring_approvals');$('#wiring').innerHTML=w.queue.map(a=>`<div class="li">${labelPill(a.status)}<b>${a.title}</b><br><span style="color:var(--dim)">${a.why}</span></div>`).join('')}
...
function ckBtns(it){return `<div class="row" style="margin-top:6px">
 ...
 <button class="btn gold" style="flex:1;padding:9px;font-size:13px" onclick="ck('signoff','${it.id}')">✍️ Sign off</button>
...
const CKSAY={done:"I've marked that done, Ross.",clear:"I've cleared it from the desk, but kept the history.",snooze:"I've snoozed that for you.",needs_approval:"That needs your approval before anyone acts.",signoff:"Signed off. Recorded, not executed.",watch:"I'll keep watching that."};
async function ck(a,id){const r=['done','clear','snooze','signoff'].includes(a)?a:'update';await post('/api/checklist/'+r,{id:id,action:a});const say=CKSAY[a]||(a+' done');toast(say);speak(say,{interrupt:true});loadChecklist();}
...
async function ckDraft(id){const title=_ckItems[id]||id;await post('/api/draft_task',{title:'FROM CHECKLIST: '+title,raw_text:title,category:'checklist'});const s="I've drafted it only — not submitted.";toast(s);speak(s,{interrupt:true})}
...
// V2D voice engine — single-speaker guard, cooldown, cross-tab lock, voice settings (browser speechSynthesis only)
const TTS=('speechSynthesis' in window);
const VKEY='qsb_rc_voice';
let vcfg=Object.assign({voice:'',rate:0.9,pitch:1.0,volume:1.0,commentaryEnabled:false,overlapGuard:true,pauseWhenHidden:true,intervalMs:8000,commentaryDisabled:false,safeMode:false,textOnly:false},(()=>{try{return JSON.parse(localStorage.getItem(VKEY)||'{}')}catch(e){return{}}})());
// audio safety defaults: commentary NEVER auto-starts; listening OFF; text/safe modes off unless Ross set them.
function saveVcfg(){try{localStorage.setItem(VKEY,JSON.stringify(vcfg))}catch(e){}}
let muted=false,commTimer=null,commLines=[],commIdx=0,recog=null,listening=false;
let isSpeaking=false,lastSpokenText='',lastSpeakAt=0,otherTabSpeaking=false;
const COOLDOWN=4000;
let bc=null;try{bc=new BroadcastChannel('qsb_rc_voice_ch')}catch(e){bc=null}
if(bc){bc.onmessage=e=>{if(e.data==='speaking'){otherTabSpeaking=true;dupWarn(true);clearTimeout(window._dupT);window._dupT=setTimeout(()=>{otherTabSpeaking=false},3500);}}}
function dupWarn(on){const w=$('#dupWarn');if(w)w.style.display=on?'block':'none';}
function listVoices(){if(!TTS)return[];try{return speechSynthesis.getVoices()||[]}catch(e){return[]}}
function pickVoice(){const vs=listVoices();if(!vs.length)return null;if(vcfg.voice){const m=vs.find(v=>v.name===vcfg.voice);if(m)return m;}
 return vs.find(v=>/en[-_]?GB/i.test(v.lang)&&/female|zira|hazel|libby|sonia|aria/i.test(v.name))||vs.find(v=>/^en/i.test(v.lang))||vs[0];}
function setAvatar(s){const a=$('#avatar');if(a)a.className='rcAvatar '+(s==='watching'?'idle':s);const st=$('#avState');if(st)st.textContent=s==='speaking'?'Speaking…':s==='listening'?'Listening…':s==='watching'?'Watching…':muted?'Muted':'At desk';}
function stopAll(){if(TTS)speechSynthesis.cancel();isSpeaking=false;setAvatar('idle');}
function ttsBusy(){return TTS&&(speechSynthesis.speaking||speechSynthesis.pending);}  // browser truth (no stuck flag)
function toggleVoiceAdv(){const a=$('#voiceAdv');if(!a)return;const show=a.style.display==='none';a.style.display=show?'block':'none';if(show){populateVoices();applyVoiceUI();}}
function speak(text,opts){opts=opts||{};if(!text||muted||!TTS)return;
 if(vcfg.textOnly){const cl=$('#commentaryLine');if(cl)cl.textContent=text;return;}  // text-only mode: show, never speak
 if(vcfg.overlapGuard){ if(!opts.interrupt&&ttsBusy())return; if(!opts.interrupt&&otherTabSpeaking)return; }
 if(vcfg.pauseWhenHidden&&document.visibilityState==='hidden')return;
 try{speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(text);const v=pickVoice();if(v)u.voice=v;
  u.rate=+vcfg.rate||0.9;u.pitch=+vcfg.pitch||1.0;let vol=+vcfg.volume;if(!(vol>0))vol=1.0;u.volume=vol;
  u.onstart=()=>{isSpeaking=true;lastSpokenText=text;lastSpeakAt=Date.now();setAvatar('speaking');if(bc)bc.postMessage('speaking');voiceDebug();};
  u.onend=()=>{isSpeaking=false;setAvatar(commTimer?'watching':'idle');voiceDebug();};
  u.onerror=()=>{isSpeaking=false;setAvatar('idle');};
  speechSynthesis.speak(u);}catch(e){isSpeaking=false;}}
async function loadCommentary(){try{const d=await j('/api/commentary');commLines=d.lines||[];const cl=$('#commentaryLine');if(cl)cl.textContent=commLines[0]||'…';const c=d.counts||{};if($('#avIssues'))$('#avIssues').textContent=(c.stale||0)+(c.not_built||0);if($('#avAppr'))$('#avAppr').textContent=c.approvals;if($('#avCheck'))$('#avCheck').textContent=new Date().toLocaleTimeString();}catch(e){}}
async function speakStatus(){await loadCommentary();speak(commLines[0],{interrupt:true});}
async function speakIssue(){const d=await j('/api/issues');const i=d.issues[0];speak(i&&i.kind!=='none'?(i.sev+' issue: '+i.kind+'. '+i.detail):'No issues detected.',{interrupt:true});}
async function speakApprovals(){const d=await j('/api/wiring_approvals');speak(d.queue.length+' wiring approvals waiting. '+d.queue.slice(0,3).map(a=>a.title).join('. '),{interrupt:true});}
async function speakReport(){const d=await j('/api/latest_reports');const r=(d.reports||[])[0];speak(r?('Latest report: '+r.name+'. '+(r.verdict||'')):'No reports.',{interrupt:true});}
function speakTest(){if(TTS&&!listVoices().length){populateVoices();toast('voice not ready yet — click Test voice again');}speak("Hello Ross, this is Iris, your Skyscraper H Q receptionist. One clear voice, at your desk.",{interrupt:true});}
function commStep(){if(!commLines.length)return;if(ttsBusy()||otherTabSpeaking)return;if(Date.now()-lastSpeakAt<COOLDOWN)return;if(vcfg.pauseWhenHidden&&document.visibilityState==='hidden')return;const line=commLines[commIdx%commLines.length];const cl=$('#commentaryLine');if(cl)cl.textContent=line;speak(line);commIdx++;}
async function startCommentary(){if(vcfg.commentaryDisabled||vcfg.safeMode||vcfg.textOnly){toast('commentary is disabled (safe/text/disabled mode)');return;}await loadCommentary();if(commTimer)clearInterval(commTimer);commIdx=0;vcfg.commentaryEnabled=true;saveVcfg();commStep();commTimer=setInterval(commStep,+vcfg.intervalMs||8000);updateTone();toast('live commentary on');}
function stopCommentary(){if(commTimer){clearInterval(commTimer);commTimer=null;}vcfg.commentaryEnabled=false;saveVcfg();stopAll();updateTone();toast('commentary stopped');}
function toggleMute(){muted=!muted;const b=$('#muteBtn');if(b)b.textContent=muted?'🔈 Unmute':'🔇 Mute';if(muted)stopAll();setAvatar('idle');updateTone();toast(muted?'muted':'unmuted');}
function killAllAudio(){if(commTimer){clearInterval(commTimer);commTimer=null;}if(TTS)speechSynthesis.cancel();if(recog){try{recog.stop()}catch(e){}}listening=false;muted=true;const b=$('#muteBtn');if(b)b.textContent='🔈 Unmute';const lb=$('#listenBtn');if(lb)lb.textContent='🎤 Start listening';setAvatar('idle');updateTone();toast('🔴 all Receptionist audio killed');}
function vtoggle(k){vcfg[k]=!vcfg[k];saveVcfg();if(k==='textOnly'&&vcfg.textOnly)stopAll();if(k==='commentaryDisabled'&&vcfg.commentaryDisabled&&commTimer){clearInterval(commTimer);commTimer=null;}if(k==='safeMode'&&vcfg.safeMode&&commTimer){clearInterval(commTimer);commTimer=null;}applyAudioUI();updateTone();toast(k+': '+(vcfg[k]?'ON':'OFF'));}
function applyAudioUI(){const set=(id,v)=>{const e=$('#'+id);if(e)e.checked=v;};set('swCommDis',vcfg.commentaryDisabled);set('swSafe',vcfg.safeMode);set('swText',vcfg.textOnly);}
function updateTone(){const t=$('#toneStatus');if(!t)return;t.innerHTML=[muted?'🔇 audio muted':'🔊 audio on',(vcfg.commentaryDisabled||!commTimer)?'commentary OFF':'commentary on',vcfg.safeMode?'safe voice mode':'',vcfg.textOnly?'TEXT ONLY':'',otherTabSpeaking?'<b style="color:var(--off)">⚠ duplicate tab</b>':''].filter(Boolean).join(' · ');}
function toggleListen(){const SR=window.SpeechRecognition||window.webkitSpeechRecognition;const b=$('#listenBtn');if(!SR){if(b){b.textContent='🎤 NEEDS WIRING';b.disabled=true;}toast('speech input not supported here');return;}if(listening){if(recog)recog.stop();listening=false;if(b)b.textContent='🎤 Start listening';setAvatar('idle');return;}try{recog=new SR();recog.lang='en-GB';recog.onresult=e=>{const t=e.results[0][0].transcript;const cl=$('#commentaryLine');if(cl)cl.textContent='heard: '+t;post('/api/note',{text:'VOICE INTAKE: '+t,kind:'intake_draft'});toast('heard: '+t);};recog.onend=()=>{listening=false;if(b)b.textContent='🎤 Start listening';setAvatar('idle');};recog.start();listening=true;if(b)b.textContent='🎤 Stop listening';setAvatar('listening');}catch(e){toast('mic permission needed')}}
// voice settings UI
function populateVoices(){const sel=$('#voiceSel');if(!sel||!TTS)return;const vs=listVoices();if(!vs.length)return;sel.innerHTML='<option value="">(auto — best English)</option>'+vs.map(v=>`<option value="${v.name}" ${v.name===vcfg.voice?'selected':''}>${v.name} · ${v.lang}</option>`).join('');}
if(TTS){try{speechSynthesis.onvoiceschanged=populateVoices;}catch(e){}}
function applyVoiceUI(){const set=(id,v)=>{const e=$('#'+id);if(e){if(e.type==='checkbox')e.checked=v;else e.value=v;}};set('rate',vcfg.rate);set('pitch',vcfg.pitch);set('vol',vcfg.volume);set('ovg',vcfg.overlapGuard);set('pwh',vcfg.pauseWhenHidden);set('intv',vcfg.intervalMs);if($('#rateV'))$('#rateV').textContent=(+vcfg.rate).toFixed(2);if($('#pitchV'))$('#pitchV').textContent=(+vcfg.pitch).toFixed(2);if($('#volV'))$('#volV').textContent=(+vcfg.volume).toFixed(2);}
function vset(k,v){vcfg[k]=v;saveVcfg();applyVoiceUI();}
function resetVoice(){vcfg={voice:'',rate:0.9,pitch:1.0,volume:1.0,commentaryEnabled:false,overlapGuard:true,pauseWhenHidden:true,intervalMs:8000};saveVcfg();populateVoices();applyVoiceUI();stopAll();toast('voice reset');}
function voiceDebug(){const p=$('#voiceDebug');if(!p)return;const v=pickVoice();p.innerHTML=`speechSynthesis available: <b>${TTS?'YES':'NO'}</b><br>selected voice: <b>${v?v.name+' ('+v.lang+')':'(none)'}</b><br>speaking: <b>${TTS&&speechSynthesis.speaking}</b> · pending: <b>${TTS&&speechSynthesis.pending}</b><br>muted: <b>${muted}</b> · commentary running: <b>${!!commTimer}</b><br>overlap guard: <b>${vcfg.overlapGuard}</b> · pause when hidden: <b>${vcfg.pauseWhenHidden}</b><br>last speech: <b>${(lastSpokenText||'—').slice(0,60)}</b><br>last at: <b>${lastSpeakAt?new Date(lastSpeakAt).toLocaleTimeString():'—'}</b><br>duplicate tab: <b style="color:${otherTabSpeaking?'var(--off)':'var(--live)'}">${otherTabSpeaking?'WARNING':'no'}</b>`;p.style.display='block';}
document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='hidden'&&vcfg.pauseWhenHidden){stopAll();}});
async function signoffLatest(){const d=await j('/api/checklist');const it=(d.active||[]).find(i=>i.status==='open'||i.status==='needs_approval');if(!it)return toast('no item to sign off');await post('/api/checklist/signoff',{id:it.id});toast('signed off: '+it.title);speak('Signed off: '+it.title,{interrupt:true});if(cur==='checklist')loadChecklist();}
async function quickNote(){const t=prompt('note:');if(t){await post('/api/note',{text:t});toast('note saved')}}
...
renderTabs();go('desk');populateVoices();applyVoiceUI();applyAudioUI();updateTone();voiceDebug();loadHistory();setInterval(()=>{if(cur==='desk'||cur==='emergency')refreshDesk()},5000);setInterval(loadReports,30000);setInterval(loadHistory,15000);
</script></body></html>"""
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p == "/api/wiring_approvals":
            return self._json(200, {"ts": now_iso(), "queue": WIRING_APPROVALS})
        if p == "/api/commentary":
            return self._json(200, commentary_lines())
        if p == "/api/activity":
            ev = []
            if ACTIVITY.exists():
    ...
    def do_POST(self):
        ...
        if p == "/api/checklist/signoff":
            return self._json(200, checklist_action(b.get("id"), "signoff", b.get("note", "")))
        if p == "/api/commentary/ack":
            log_activity("commentary_ack", item=(b.get("line") or "")[:60], result="acknowledged", actor="ross")
            return self._json(200, {"ok": True})
        if p == "/api/signoff":
            field = (b.get("field") or "")[:60]
            value = (b.get("value") or "")[:60]
            log_activity("ross_signoff", item=field, result=value, actor="ross")
            return self._json(200, {"ok": True, "field": field, "value": value, "note": "Ross signoff logged; no work executed"})
