...
# ── BUILDING ELEVATION: the presence-only floors to the RIGHT of the backbone,
#    grouped by zone into contiguous columns (a fresh column per zone). ──
_ELEV2_X0 = 1820      # left edge of the elevation (clear of the provider fan @1600)
_ELEV2_COLW = 152     # column pitch (room for the right-hand label)
_ELEV2_ROWS = 14      # floors per column before wrapping (roomy row pitch = no bloom)
_ELEV2_YTOP = 150
_ELEV2_YBOT = 1190
...
_ELEV2_BLOCKS = []
_col_cur = 0
_span2 = _ELEV2_YBOT - _ELEV2_YTOP
for _z in _elev2_order:
    ...
    for _mi, _sid in enumerate(_mem):
        _cinz = _mi // _ELEV2_ROWS
        _row = _mi % _ELEV2_ROWS
        _gc = _zc0 + _cinz
        STATIONS[_sid]["x"] = _ELEV2_X0 + _gc * _ELEV2_COLW
        STATIONS[_sid]["y"] = int(_ELEV2_YTOP + _row * (_span2 / (_ELEV2_ROWS - 1)))
    _zcols = (len(_mem) + _ELEV2_ROWS - 1) // _ELEV2_ROWS
    _col_cur = _zc0 + _zcols
    _ELEV2_BLOCKS.append({
        "zone": _z, "color": ZONE_COLORS.get(_z, "#5a6577"),
        "x0": _ELEV2_X0 + _zc0 * _ELEV2_COLW - 30,
        "x1": _ELEV2_X0 + (_col_cur - 1) * _ELEV2_COLW + 118,   # +right label margin
        "count": len(_mem),
    })
...
GRID_VIEW_H = max(_ELEV2_YBOT, 1010) + 60
GRID_VIEWBOX = f"0 0 {GRID_VIEW_W} {GRID_VIEW_H}"
