...
def cmd_append(args):
    ...
    if not p.is_absolute():
        p = REPO / p
    p.parent.mkdir(parents=True, exist_ok=True)
    # ── Ross 2026-07-07 append-only MEMORY SAFETY guard ──
    # Never truncate. Record size/lines before + after; if the file shrank,
    # fail loudly with MEMORY SAFETY FAILURE (do not silently lose old notes).
    before_bytes = p.stat().st_size if p.exists() else 0
    before_lines = sum(1 for _ in p.open()) if p.exists() else 0
    if content and not content.endswith("\n"):
        content += "\n"
    with open(p, "a") as f:            # append mode only — "a" never truncates
        f.write(content)
        f.flush()
        os.fsync(f.fileno())
    ...
    if after_bytes < before_bytes or after_lines < before_lines:
        journal({"op": "append", "path": str(p), "ok": False,
                 "err": "MEMORY SAFETY FAILURE: file shrank on append",
                 "before_bytes": before_bytes, "after_bytes": after_bytes,
                 "before_lines": before_lines, "after_lines": after_lines})
        print(json.dumps({"ok": False, "err": "MEMORY SAFETY FAILURE",
                          "before_bytes": before_bytes, "after_bytes": after_bytes,
                          "before_lines": before_lines, "after_lines": after_lines}))
        return 2
