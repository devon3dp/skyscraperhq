...
MEMBERS = {
    ...
    "claude":   {"label": "Claude",   "hue": 30,  "color": "#cd7f45", "role": "Bill · Concierge"},
    ...
    # ── ALIASES ──
    "hq":       {"label": "Bill",       "hue": 30,  "color": "#cd7f45", "role": "Bill · Concierge"},
...
_P1_DASH_ORIG = {"tp": "http://192.168.1.74:9110/",
                 "acer": "http://192.168.1.78:9000/"}  # Acer dashboard discovered 2026-07-11 (Phase 2C)
...
def p1_truth_model():
    ...
    for k, name in [("tp", "TP-Pip"), ("acer", "Acer-Cass")]:
        ...
        # PHASE 2 — dynamic registration overlay. Registration alone must NEVER turn a
        # physical badge green; a fresh direct probe is still required.
        worker_id = {"tp": "tp_pip", "acer": "acer_cass"}[k]
        reg = None
        try:
            import qsb_physical_worker_registry as _pwr
            reg = _pwr.get_one(worker_id)
        except Exception:
            reg = None
        if reg and reg.get("registration_fresh"):
            registration = {"state": "FRESH", "source_ip": reg.get("source_ip"),
                            "runtime_port": reg.get("runtime_port"), "dashboard_port": reg.get("dashboard_port"),
                            "heartbeat_age_s": reg.get("heartbeat_age_s"),
                            "active_adapter": reg.get("active_adapter"), "connection_type": reg.get("connection_type"),
                            "source": "PHYSICAL-WORKER REGISTRY (dynamic heartbeat)"}
            registered_status = "PHYSICAL WORKER LIVE" if rt == "LIVE" else "REGISTERED — ENDPOINT UNREACHABLE"
        elif reg:
            registration = {"state": "STALE", "source_ip": reg.get("source_ip"),
                            "heartbeat_age_s": reg.get("heartbeat_age_s"),
                            "source": "PHYSICAL-WORKER REGISTRY (expired heartbeat)"}
            registered_status = "LAST KNOWN ENDPOINT STALE"
        else:
            registration = {"state": "NONE", "source": "no dynamic heartbeat received yet"}
            registered_status = ("CURRENT ENDPOINT UNKNOWN via registry — using CONFIGURED endpoint "
                                 "(direct probe still authoritative)")
        # original dashboard: TP has a discovered remote dash (:9110); Acer's is Ross-confirmed local, endpoint unknown
        if k in _P1_DASH_ORIG:
            od = _p1_http("dash_orig_" + k, _P1_DASH_ORIG[k])
            title = _re.search(r'<title>([^<]+)</title>', od.get("body_sample", "") or "")
            original_dashboard = {"endpoint": _P1_DASH_ORIG[k], "state": od["state"],
            ...
            original_dashboard = {"endpoint": "UNKNOWN", "state": "LOCAL DASHBOARD ROSS-CONFIRMED",
                                  ...
                                  "note": "runtime port is the API surface, NOT the dedicated dashboard"}
        entry = {
            "name": name,
...
<!-- Ross 2026-07-06: Bill stats strip on iPad. Live via /hq/stats. ADD not TAKE. -->
...
<div class=section id=sec-revs style="scroll-margin-top:70px"><div class=section-title>🎛️ rev gauges · CEO velocity</div>
  <div style='padding:12px;background:#0e1420;border:1px solid #22334a;border-radius:12px'>
    <div id=rev-panel style='display:grid;grid-template-columns:repeat(4,1fr);gap:10px'>
      <div class=rev-tile data-ceo=bill style='background:#000;padding:10px;border-radius:10px;border:1px solid #22334a;text-align:center'>
        <div style='color:#eab308;font-size:11px;font-weight:800'>Bill</div>
        <svg viewBox='0 0 100 60' style='width:100%;height:60px'>
          ...
          <path id=rev-bill d='M 10 50 A 40 40 0 0 1 90 50' stroke='#eab308' stroke-width='6' fill='none' stroke-dasharray='0 200'/>
          <text id=rev-bill-txt x='50' y='45' fill='#eab308' font-size='16' text-anchor='middle' font-weight='900'>0</text>
        </svg>
...
<div class=section id=sec-actions style="scroll-margin-top:70px"><div class=section-title>⚡ MORE actions</div>
  <button class='btn' onclick='assignRandom("bill")'>📋 assign Bill a task</button>
  ...
  <div style='padding:12px;background:#0e1420;border:1px solid #22334a;border-radius:12px'>
    ...
    <div style='display:flex;gap:8px;flex-wrap:wrap'>
      <select id=new-task-assign style='background:#0b1220;color:#e8ecf3;border:1px solid #22334a;border-radius:8px;padding:10px;flex:1'>
        <option value=''>assign to...</option><option value=bill>Bill</option><option value=wren>Wren</option><option value=tp_pip>TP-Pip</option><option value=acer_cass>Acer-Cass</option>
      </select>
...
<div class=section id=sec-voice style="scroll-margin-top:70px"><div class=section-title>🎤 voice command center · listen + speak</div>
  <div style='padding:14px;background:#0e1420;border:1px solid #22334a;border-radius:14px'>
    <div style='display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px'>
      <button class='btn voice-btn' data-ceo=bill style='min-height:72px;background:#eab308;color:#000;font-weight:900;font-size:16px;border:none' onclick='voiceChat("bill","Bill")'>🎤 Bill</button>
...
<div class=section id=sec-chat style="scroll-margin-top:70px"><div class=section-title>💬 chat with council</div>
  <div style='padding:12px;background:#0e1420;border:1px solid #22334a;border-radius:12px'>
    <div style='display:flex;gap:6px;flex-wrap:wrap;margin-bottom:8px'>
      <button class='btn' style='min-height:44px;flex:1;background:#eab308;color:#000' onclick='setChatTarget("bill","Bill")'>Bill</button>
    ...
    <div id=chat-target style='font-size:11px;color:#94a3b8;margin-bottom:6px'>target: <b style='color:#eab308'>Bill</b></div>
  ...
  <div style='padding:12px;background:#0e1420;border:2px solid #f59e0b;border-radius:12px'>
    <div style='display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap'>
      ...
      <select id=notice-from style='background:#0b1220;color:#e8ecf3;border:1px solid #22334a;border-radius:8px;padding:10px'>
        <option value=ross>⭐ Ross</option><option value=bill>Bill</option><option value=wren>Wren</option>
...
async function tick(){
  try{
    ...
    (ts.messages||[]).forEach(m => {
      const w = m.who === 'hq' ? 'bill' : m.who === 'tp' ? 'tp_pip' : m.who === 'acer' ? 'acer_cass' : m.who;
    ...
    document.getElementById('commentary').innerHTML = ordered.map(m => {
      const cfg = CEO_META[m.who === 'hq' ? 'bill' : m.who === 'tp' ? 'tp_pip' : m.who === 'acer' ? 'acer_cass' : m.who] || {n:m.who,c:'#94a3b8'};
...
// #187 CHAT
let CHAT_TARGET = 'bill';
const CHAT_LABELS = {bill:'Bill',wren:'Wren',tp_pip:'TP-Pip',acer_cass:'Acer-Cass',ALL:'All CEOs'};
...
async function _sendChatInner(){
  ...
  // Route to CEO(s)
  const targets = CHAT_TARGET === 'ALL' ? ['bill','wren','tp_pip','acer_cass'] : [CHAT_TARGET];
  for(const ceo of targets){
    const cfg = {bill:'#eab308',wren:'#a78bfa',tp_pip:'#22d3ee',acer_cass:'#f59e0b'}[ceo];
...
function _pickVoice(ceo){
  ...
  const preferMale = ceo==='bill' || ceo==='tp_pip';
  ...
  const idx = {bill:0,wren:1,tp_pip:2,acer_cass:3}[ceo] ?? 0;
...
function speak(text, ceo){
  ...
  try{
    ...
    u.rate = 1.05;
    u.pitch = ceo==='wren'?1.25:ceo==='bill'?0.9:ceo==='tp_pip'?1.05:ceo==='acer_cass'?1.15:1.0;
...
async function voicePreset(text){
  const target = ACTIVE_CEO_ID || 'bill';
  const name = {bill:'Bill',wren:'Wren',tp_pip:'TP-Pip',acer_cass:'Acer-Cass',ALL:'All 4 CEOs'}[target] || target;
...
async function _dispatchVoiceText(text, ceoId, name){
  ...
  const targets = ceoId==='ALL'?['bill','wren','tp_pip','acer_cass']:[ceoId];
  for(const c of targets){
    try{
      ...
      const d = await r.json();
      const col = {bill:'#eab308',wren:'#a78bfa',tp_pip:'#22d3ee',acer_cass:'#f59e0b'}[c] || '#e8ecf3';
      const nm = {bill:'Bill',wren:'Wren',tp_pip:'TP-Pip',acer_cass:'Acer-Cass'}[c] || c;
...
function toggleLiveVoice(){
  ...
  if (LVC_ON) {
    ...
    fetch('/talk/data', {cache:'no-store'}).then(r=>r.json()).then(d=>{
      ...
      chime();
      _lvcSpeak('Live voice commentary on. I will read each new message.', 'bill');
    }).catch(()=>{});
  } else {
...
function _lvcVoice(who){
  ...
  if (who === 'bill' || who === 'tp_pip') {
    const m = pool.find(v => male.test(v.name)); if (m) return m;
  }
  const idx = {bill:0, wren:1, tp_pip:2, acer_cass:3, ross:4}[who] ?? 0;
  return pool[idx % pool.length];
}
function _lvcSpeak(text, who){
  try {
    ...
    u.rate = LVC_RATE;
    u.pitch = who === 'wren' ? 1.25 : who === 'bill' ? 0.9 :
...
async function _lvcTick(){
  ...
  try {
    ...
    const nameFor = {bill:'Bill', wren:'Wren', tp_pip:'TP-Pip', acer_cass:'Acer-Cass', ross:'Ross', hq:'Bill', tp:'TP-Pip', acer:'Acer-Cass'};
...
async function loadChatHistory(){
  ...
  try{
    ...
    const nameFor  = {ross:'⭐ Ross', hq:'Bill', wren:'Wren', tp:'TP-Pip', acer:'Acer-Cass'};
    const chat = msgs
...
async function revTick(){
  try{
    ...
    const counts = {bill:0,wren:0,tp_pip:0,acer_cass:0};
...
async function checklistTick(){
  try{
    ...
    const ceoCol = {bill:'#eab308',wren:'#a78bfa',tp_pip:'#22d3ee',acer_cass:'#f59e0b'};
...
async function noticeTick(){
  try {
    ...
    const notes = d.notes || [];
    const cf = {ross:'#facc15',bill:'#eab308',wren:'#a78bfa',tp_pip:'#22d3ee',acer_cass:'#f59e0b',max_rally:'#ec4899',anon:'#94a3b8'};
...
async function evolutionTick(){
  try {
    ...
    const ceos = d.ceos || [];
    const cf = {bill:'#eab308',wren:'#a78bfa',tp_pip:'#22d3ee',acer_cass:'#f59e0b'};
    const name = {bill:'HQ',wren:'Wren',tp_pip:'TP',acer_cass:'Acer'};
...
<div class=grid>
  ...
  <div class=section>
    <h2>👥 meet the CEOs</h2>
    <div class=ceo-row>
      <div class="ceo hq"><div class=name>Bill</div><div class=role>orchestrator · Claude API mind · watches over the tower</div></div>
...
const TOUR_SCRIPT = [
  ...
  "Meet our four CEOs. Bill is the orchestrator. Wren is the designer and kernel mind. TP-Pip runs research. Acer-Cass runs the data foundry. They work together, and separately.",
...
async function tourTick(){
  try {
    ...
    const ceosLive = (an.links||[]).filter(l => ['Bill','Wren','TP-Pip','Acer-Cass'].includes(l.name) && l.ok).length;
...
<div class=grid>
  ...
  <div class=section id=stop-ceos>
    ...
    <div class=ceo-row>
      <div class="ceo hq"><div class=name>🏛 Bill</div><div class=role>The orchestrator. Runs on the Claude account mind, sees the whole tower at once, and coordinates the council — pulling every job from the shared Task Council queue and making sure nothing moves without a task behind it.</div></div>
...
const IRIS_SCRIPT = {
  ...
  ceos: "Meet the four minds. Bill is the orchestrator, running on the Claude API. Wren is the designer, on local qwen. TP-Pip does research and audit from her ThinkPad. Acer-Cass runs the data foundry from his Acer. Each mind lives on its own hardware but shares the town-square.",
...
<div class=filter-chips>
  ...
  <button class="chip" data-filter="bill" onclick="setFilter('bill')">Bill</button>
...
<script>
const CEO = {
  bill: {name:'Bill', color:'#eab308', initial:'H'},
...
const P = {
  ...
  tp:   {title:'TP-Pip',     sub:'iframe = PHYSICAL Cockpit V2 .74:9120 · legacy /proxy/tp_legacy · surrogate /proxy/tp_surrogate',brain:'local llama3.2 (CPU)', proxy:'/proxy/tp',   direct:()=>'http://'+location.hostname+':8852/proxy/tp'},
  acer: {title:'Acer-Cass',  sub:'iframe = PHYSICAL Cockpit V2 .78:9120 · legacy /proxy/acer_legacy · surrogate /proxy/acer_surrogate',brain:'local + gene', proxy:'/proxy/acer', direct:()=>'http://'+location.hostname+':8852/proxy/acer'},
};
...
const BCAST=[['bill','Claude HQ'],['wren','Wren'],['tp_pip','TP-Pip'],['acer_cass','Acer-Cass']];
function broadcast(){
  ...
  for(const [id,name] of BCAST){
    ...
    const url = (id==='bill') ? '/api/times4/ask_claude_hq' : '/ceo_mind/'+id;
...
function renderTruth(d){
  ...
  for(const k of ['acer','tp']){
    ...
    html+=card(c.name, c.operating_mode,
      ...
      '<div style="margin-top:3px">REGISTRATION: '+_b(c.registration.state==='FRESH'?'LIVE':(c.registration.state==='STALE'?'PARTIAL':'UNREACHABLE'))+' <span style="color:#64748b">'+c.registered_status+((c.registration.connection_type)?(' · '+c.registration.connection_type+' · hb '+((c.registration.heartbeat_age_s!=null)?c.registration.heartbeat_age_s+'s':'-')):'')+'</span></div>'+
      hist);
  }
...
def _council_snapshot() -> dict:
    """Council of Four live view — Bill, Wren, TP-Pip, Acer-Cass.
    ...
    try:
            ...
            try:
                ...
                if ts.exists():
                    for line in reversed(ts.read_text().splitlines()[-40:]):
                        try:
                            d = json.loads(line)
                            if (d.get("from") or d.get("who") or "").lower() == "bill":
                                t = (d.get("text") or "").strip()
                                if t and not t.startswith(("bill said:", "Corrected", "Heard")):
        ...
        out["members"].append({
            "name": "Bill", "role": "Coordinator, Floor 47",
        ...
        out["members"].append({"name":"Bill","status":"err","detail":str(e)[:100]})
...
<div class=sub>Live view — Ross · Bill · Wren · TP-Pip · Acer-Cass · 5 minds locked · auto-refresh 1s</div>
...
<script>
const CEO_META = {
  bill:{name:'Bill', color:'#eab308', initial:'H', role:'Founding Ops'},
  ...
  try{
    ...
    if(tiles){
      ...
      tiles.innerHTML = Object.entries(CEO_META).map(([id,m]) => {
        ...
        const talk = recentByCeo[id === 'bill' ? 'hq' : id === 'tp_pip' ? 'tp' : id === 'acer_cass' ? 'acer' : id];
    ...
    if(cel){
      const WHO_LABELS = {hq:'Bill', wren:'Wren', tp:'TP-Pip', acer:'Acer-Cass', ross:'Ross', watcher:'Council Watcher'};
...
async function tsTick(){
  try {
    ...
    const cmap = {ross:'#3b82f6',bill:'#eab308',wren:'#a78bfa',tp_pip:'#22d3ee',acer_cass:'#f59e0b',council:'#94a3b8'};
...
<script>
const COLORS = {bill:'#eab308', wren:'#a78bfa', tp_pip:'#22d3ee', acer_cass:'#f59e0b',
                ross:'#e8ecf3', ross_via_hq:'#e8ecf3', hermes:'#3b82f6', iquest:'#10b981'};
const INITIAL = {bill:'HQ', wren:'W', tp_pip:'TP', acer_cass:'AC',
...
<div class=quorum-strip>
  ...
  <div class=q-item><span class=q-dot id=d-hq></span>Bill</div>
...
<div class=grid>
  <div class=pane id=p-hq><h3>🟨 Bill <span class=conn id=c-hq>?</span></h3><div class=thoughts id=t-hq></div>
    <div class=commentary-row>
      <button class=cbtn onclick="say('bill','status update: OANDA trader live, EUR/USD long')">🗣 status update</button>
      <button class=cbtn onclick="say('bill','quorum check running')">🗣 quorum check</button>
      <button class=cbtn onclick="say('bill','all systems nominal')">🗣 nominal</button>
    </div>
  </div>
...
/* --- Bill's 3 animations (Ross 2026-07-05 asked for 3 more) --- */
...
<a href='/ipad' style='position:fixed;bottom:16px;right:16px;background:#eab308;color:#000;padding:14px 18px;border-radius:50%;text-decoration:none;font-weight:900;box-shadow:0 6px 16px rgba(0,0,0,0.6);z-index:9999;font-size:22px'>🏠</a>
<!-- Ross 2026-07-06: Bill stats strip. Live via /hq/stats. ADD not TAKE. -->
<div id=hq-stats-strip style="background:#0e1420;border:1px solid #eab30840;border-radius:8px;padding:10px 14px;margin-bottom:12px;display:flex;gap:18px;flex-wrap:wrap;font-family:ui-monospace,monospace;font-size:11.5px;color:#94a3b8">
  <div><span style="color:#eab308;font-weight:800">🧠 Bill</span> · rank <b id=hq-rank style="color:#e8ecf3">?</b> · brain <b id=hq-brain style="color:#e8ecf3">?</b></div>
...
const CALLER_COLORS = {bill:'#eab308', wren:'#a78bfa', tp_pip:'#22d3ee', 'TP-Pip':'#22d3ee', acer_cass:'#f59e0b', 'Acer-Cass':'#f59e0b', ross:'#e8ecf3', distiller_wren:'#a78bfa', distiller_tp_pip:'#22d3ee', distiller_acer_cass:'#f59e0b', distiller_bill:'#eab308', verify_wrap:'#ec4899'};
function callerNick(n){const M={wren:'Wren',tp_pip:'Pip',acer_cass:'Asa',bill:'Bill',ross:'Ross',diagnostic:'diag',distiller_wren:'d·Wren',distiller_tp_pip:'d·Pip',distiller_acer_cass:'d·Asa',distiller_bill:'d·Bill'};return M[n]||(n||'?').replace(/^distiller_/,'d·')}  /* 2026-07-18: canonical names to agree with V4 */
...
const STAGE_PCT = {open:5, submitted:8, pending_admission:10, assigned:15, acknowledged:30, claimed:40, routed_for_work:48, in_progress:55, needs_verification:70, needs_independent_verification:72, awaiting_peer_signoff:75, needs_Ross_ChatGPT_verifier_review:82, ready_to_ship:90, done:100, cancelled:100, denied:100, blocked:60};
function matchFilter(t, f){
  const st = t.state;
  if(f === 'all') return true;
  if(f === 'open') return st === 'open' || st === 'submitted' || st === 'assigned';
  if(f === 'in-flight') return ['claimed','acknowledged','in_progress','routed_for_work'].includes(st);
  if(f === 'awaiting') return ['awaiting_peer_signoff','ready_to_ship','needs_verification','needs_independent_verification','needs_Ross_ChatGPT_verifier_review'].includes(st);
...
const OWNERS = {bill:'#eab308', wren:'#a78bfa', tp_pip:'#22d3ee', acer_cass:'#f59e0b', ross:'#e8ecf3'};
async function tick(){
  try{
    ...
    const ceos = ['bill','wren','tp_pip','acer_cass','ross'];
    const stats = {};
  ...
  return `<div class="col competition" style="border-top:3px solid #eab308"><h2>🏆 COMPETITION<span class=cnt>6 rules</span></h2>
    <div style="padding:8px;font-size:11px;color:#cbd5e1;line-height:1.5;">
      ...
      <div id="qualif-status" style="margin-top:8px;padding-top:8px;border-top:1px solid #1f2937;font-size:10.5px;">
        <div><span style="color:#eab308">●</span> Bill · The Beacon Hall (gold)</div>
...
function taskCard(t){
  ...
  const partnerCandidates = ['bill','wren','tp_pip','acer_cass'].filter(c => c !== t.owner);
...
def _call_peer_ceo(ceo: str, prompt: str, timeout_s: int = 30) -> dict:
    ...
    if cfg["kind"] == "ssh_curl":
        body = json.dumps({"from": "bill_via_ipad", "text": prompt})
...
class after animation
    ...
    if (esEl) {
      const eventColors = {wren:'#f97316', tp:'#22d3ee', 'thinkpad-ceo':'#22d3ee', acer:'#f59e0b', bill:'#eab308', claude:'#eab308', hermes:'#a78bfa', forge:'#e879f9'};
...
function renderCommentary(list) {
  ...
  const WHO_LABELS = {hq:'Bill', wren:'Wren', tp:'TP-Pip', acer:'Acer-Cass', ross:'Ross', watcher:'Council Watcher', bill:'Bill', tp_pip:'TP-Pip', acer_cass:'Acer-Cass'};
  const WHO_COLORS_CEO = {hq:'#eab308', wren:'#a78bfa', tp:'#22d3ee', acer:'#f59e0b', ross:'#e8ecf3', watcher:'#ef4444', bill:'#eab308', tp_pip:'#22d3ee', acer_cass:'#f59e0b'};
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            if self.path in ("/proxy/hq","/proxy/wren","/proxy/tp","/proxy/acer",
                             "/proxy/tp_legacy","/proxy/acer_legacy",
                             "/proxy/tp_surrogate","/proxy/acer_surrogate",
                             "/proxy/oracle","/proxy/lumen","/proxy/studio","/proxy/traders_live"):
                # 2026-07-11 TP-PIP-BRAIN-TRUTH: /proxy/tp now serves the PHYSICAL ThinkPad
                # dashboard (.74:9110), NOT the HQ surrogate. The surrogate is a SEPARATE,
                # clearly-labelled proxy (/proxy/tp_surrogate). Same split for acer.
                _proxy_map = {"/proxy/hq":"http://127.0.0.1:8850/",
                              "/proxy/wren":"http://127.0.0.1:8851/",
                              "/proxy/tp":"http://192.168.1.74:9120/",              # V2 physical cockpit (main)
                              "/proxy/tp_legacy":"http://192.168.1.74:9110/",       # legacy diagnostic view
                              "/proxy/tp_surrogate":"http://127.0.0.1:8861/dash",   # HQ surrogate — NOT physical
                              "/proxy/acer":"http://192.168.1.78:9120/",            # V2 physical cockpit (main)
                              "/proxy/acer_legacy":"http://192.168.1.78:9000/",     # legacy diagnostic view
                              "/proxy/acer_surrogate":"http://127.0.0.1:8862/dash", # HQ surrogate — NOT physical
            ...
            if self.path == "/api/hq_identity":
                # MASTER PHASE 2 — workers confirm they found the REAL HQ before registering.
                self._send_json(200, {"id": "skyscraperhq_boardroom", "service": "qsb_boardroom",
                                      "port": 8852, "physical_worker_registry": True, "ts": utc_iso()}); return
            if self.path == "/api/physical_workers":
                import qsb_physical_worker_registry as _pw
                self._send_json(200, _pw.get_all()); return
            if self.path == "/api/physical_workers/health":
                import qsb_physical_worker_registry as _pw
                self._send_json(200, _pw.health()); return
            if self.path.startswith("/api/physical_workers/"):
                import qsb_physical_worker_registry as _pw
                wid = self.path.rsplit("/", 1)[-1]
                w = _pw.get_one(wid)
                self._send_json(200 if w else 404, w or {"error": "not_found", "worker_id": wid}); return
            ...
            if self.path == "/team_live/data":
                ...
                for ceo in ("bill","tp_pip","acer_cass","wren"):
                    try:
                        ...
                        out["cards"][ceo] = []
                # 4. Connection status
                _peers = {"bill":"127.0.0.1:8850","tp_pip":"192.168.1.74:8871",
            ...
            if self.path == "/trading/data":
                ...
                # Aggregate per CEO
                _venue_ceo = {"oanda_practice":"bill","binance_testnet":"tp_pip","alpaca_paper":"acer_cass"}
                _mid = {"bill":{"venue":"oanda_practice","symbol":"EUR_USD","rounds":0,"last_side":"","last_fill_px":"","live_px":"","pnl_pips":""},
                ...
                try:
                    ...
                    _req = _u.Request(f"https://api-fxpractice.oanda.com/v3/accounts/{_acct}/pricing?instruments={_mid['bill']['symbol']}",
                                      headers={"Authorization":f"Bearer {_tok}"})
                    _r = json.loads(_u.urlopen(_req, timeout=6).read())
                    _p = _r["prices"][0]
                    _bid = float(_p["bids"][0]["price"]); _ask = float(_p["asks"][0]["price"])
                    _mid["bill"]["live_px"] = f"{_bid:.5f}/{_ask:.5f}"
                    _fp = _mid["bill"]["last_fill_px"]
                    if _fp and _mid["bill"]["last_side"] == "BUY":
                        _pips = round((_bid - float(_fp)) * 10000, 1)
                        _mid["bill"]["pnl_pips"] = f"{_pips:+.1f} pips"
                except Exception as _e: _mid["bill"]["live_px"] = f"err:{str(_e)[:30]}"
                # Binance
                try:
            ...
            if self.path == "/hq/stats":
                ...
                out = {"ts": utc_iso(), "who":"bill", "rank": 4,
                       "brain":"qwen2.5:14b (MacBook)", "floor":"Concierge · MacBook",
                       "pid": _os.getpid()}
                try:
                    _card = json.load(open(ROOT / "data/registries/qsb_bill_operator_card.json"))
            ...
            if self.path == "/link_health":
                ...
                targets = [
                    {"name":"Bill","url":"http://127.0.0.1:8850/","hint":"HQ dash 8850"},
            ...
            if self.path == "/diagnostics":
                ...
                # 9) brain router smoke — 2026-07-18 Ross: reachability ONLY, no paid LLM ping.
                # (was _rt("ping"...) which routed to paid DeepSeek every smoke, burning credit.)
                try:
                    import sys as _sys
                    _sys.path.insert(0, "/vaults/nvme0/qsb_tower_v1/tools")
                    ...
                    try:
                        ...
                        _v4 = json.loads(_u.urlopen("http://127.0.0.1:8860/health", timeout=3).read()).get("ok", False)
                    except Exception: pass
                    add("brain router", hasattr(_br, "route") and _v4, "V4 :8860 healthy (no paid ping)")
            ...
            if self.path == "/evolution":
                ...
                for t in snap.get("tasks",[]):
                    ...
                    if c not in ("bill","wren","tp_pip","acer_cass"): continue
                    try:
                ...
                out = []
                for c in ("bill","wren","tp_pip","acer_cass"):
            ...
            if self.path == "/teamwork":
                ...
                CEOS = ["bill","wren","tp_pip","acer_cass"]
    ...
    def do_POST(self):
        # MASTER PHASE 2 — physical-worker registry. Source IP is taken from the
        # TCP connection, NEVER trusted from the JSON body. Validation + rejection
        # live in tools/qsb_physical_worker_registry.py.
        try:
            _pw_path = getattr(self, "path", "")
            if _pw_path in ("/api/physical_workers/register", "/api/physical_workers/heartbeat"):
                import qsb_physical_worker_registry as _pw
                ...
                try:
                    ...
                    payload = {}
                src_ip = self.client_address[0] if getattr(self, "client_address", None) else ""
                out = (_pw.register if _pw_path.endswith("/register") else _pw.heartbeat)(payload, src_ip)
                self._send_json(200 if out.get("ok") else 400, out); return
        except Exception as e:
            self._send_json(500, {"ok": False, "error": repr(e)[:200]}); return
        ...
        try:
            ...
            if self.path.startswith("/ceo_mind/"):
                ...
                try:
                    if ceo in ("hq","bill"):
                        # Bill JACKETS: forward to his dash /api/ask_hq (gene-pool default,
                        ...
                        try:
                            ...
                            try:
                                ...
                                self._send_json(200, {"reply": (_rep or "").strip(),
                                                      "mind": "gene_pool_fallback(hq_dash_down)", "ceo": "bill",
                        ...
                        self._send_json(400, {"error": "unknown ceo", "allowed": ["bill","wren","tp_pip","acer_cass"]}); return
                except Exception as e:
            ...
            if self.path == "/hq/write_file":
                ...
                # Whitelist: only tp_pip, acer_cass, wren, bill sandboxes
                if ceo not in ("tp_pip","acer_cass","wren","bill"):
                    self._send_json(400, {"error":"unknown ceo","allowed":["tp_pip","acer_cass","wren","bill"]}); return
            ...
            if self.path == "/append_card_ledger":
                ...
                entry = payload.get("entry") or {}
                if ceo not in ("bill","wren","tp_pip","acer_cass"):
                    self._send_json(400, {"error":"ceo must be bill|wren|tp_pip|acer_cass"}); return
