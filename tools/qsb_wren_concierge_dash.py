...
def team():
    ...
    return {"ts": now_iso(), "members": [
        ...
        {"name": "Wren", "role": "OBSERVER / GUARDIAN / concierge", "endpoint": f"{HQ_IP}:8851",
         ...
         "can_execute": False, "can_verify": True, "can_self_close": False, "blockers": "draft-only (Stage B: :8851 now single systemd owner)"},
...
def task_council_bridge():
    # 2026-07-11 freshness fix: judge from the LIVE read-model snapshot (qsb_council_tasks_snapshot.json,
    # maintained by the Boardroom) via its own `ts`, NOT the raw append-log mtime. The raw log is idle-old
    # when no NEW tasks are created — that is not real staleness. Falls back to the raw log if no snapshot.
    SNAP = REG / "qsb_council_tasks_snapshot.json"
    states = Counter(); total = 0; age = None; source = "snapshot"
    snap = None
    try:
    ...
    if snap and snap.get("ts"):
        ...
        try:
            for t in snap.get("tasks", []):
                states[t.get("state", "?")] += 1
        except Exception:
            pass
        try:
            from datetime import datetime as _dt
            st = _dt.fromisoformat(str(snap["ts"]).replace("Z", "+00:00"))
            age = int(time.time() - st.timestamp())
        except Exception:
            age = int(time.time() - SNAP.stat().st_mtime) if SNAP.exists() else None
    elif COUNCIL.exists():
        source = "raw_log"
        try:
    ...
    return {"ts": now_iso(), "source": source, "total": total, "by_state": dict(states), "open": open_n,
            "needs_approval": states.get("needs_approval", 0) + states.get("needs_Ross_ChatGPT_verifier_review", 0),
            "age_s": age, "stale": stale, "tp_available": tpu, "acer_available": acu,
            "note": "Freshness from live snapshot read-model. Wren may DRAFT + mark ready-for-Ross; she does NOT submit/claim/close tasks or fake peer signoff."}
...
def past_present_future():
    ...
    return {"ts": now_iso(),
            ...
            "future": {"next_approval": briefing()["next_safest"],
                       "risk": "Wren's own LLM self-assessment is advisory, not authoritative — use /api/state live probes. Keep her draft-only. (Stage B resolved the :8851 launcher thrash: single systemd owner.)",
...
def _annotate_proof(jc):
    # Compute signoff_type + proof_status LIVE from real fields only — never invents proof.
    has_pkt = bool(jc.get("approval_packet_id"))
    has_rpt = bool(jc.get("latest_report_path"))
    has_ev = bool(jc.get("evidence_paths"))
    has_tc = bool(jc.get("task_council_task_id"))
    record_only = (jc.get("category") in ("dashboard_read_model_repoint", "read_only_live_verification", "general", "checklist")) and not has_tc
    st = jc.get("status")
    if st == "signed_off":
        if has_rpt or has_ev:
            typ = "RECORD_ONLY_SIGNED_OFF_WITH_REPORT" if record_only else "ROSS_SIGNED_OFF_WITH_EVIDENCE"
            proof = "ok"
        elif has_pkt and record_only:
            typ, proof = "RECORD_ONLY_SIGNED_OFF", "record_only_no_report"
        else:
            # signed off during smoke/use testing with no packet/report/evidence/council-id
            typ, proof = "TEST_ARTIFACT_EVIDENCE_MISSING", "missing"
    elif st == "ready_for_ross_signoff":
        typ = "WORK_REPORTED" if (has_rpt or has_ev) else "READY_FOR_ROSS_SIGNOFF"
        proof = "ok" if (has_rpt or has_ev) else ("record_only" if record_only else "missing")
    elif st in ("submitted_to_task_council", "ready_for_task_council"):
        typ = "TASK_COUNCIL_SUBMITTED" if has_tc else "JOB_READY_FOR_WORK"
        proof = "ok" if has_tc else "pending"
    elif st == "approved_by_ross":
        typ, proof = ("APPROVAL_PACKET_CREATED" if has_pkt else "UI_DECISION_ONLY"), "pending"
    else:
        typ, proof = None, "pending"
    jc["signoff_type"] = typ
    jc["proof_status"] = proof
    jc["proof"] = {"packet": has_pkt, "R115": bool(jc.get("R115_manifest_required")), "report": has_rpt,
                   "evidence": has_ev, "council_id": has_tc, "record_only": record_only}
    # test artifacts (signed off with no proof) drop out of the normal completed view but STAY in history
    is_artifact = (typ == "TEST_ARTIFACT_EVIDENCE_MISSING")
    jc["visible_in_normal_completed"] = not is_artifact
    jc["visible_in_history"] = True
    jc["warning"] = ("Test artifact — signed off during smoke/use testing, no work evidence attached." if is_artifact else None)
    return jc


def load_jobs():
    return [_annotate_proof(j) for j in read_tail(JOBS, 300)]
...
# ============================================================================
# STAGE C — truthful component state (real probes, not the local model's guess),
# operating mode (OBSERVE / ASSIST / AUTOPILOT-disabled), and Ross decision/history
# stores. GATE C0: Wren's own briefing MUST source status from here, NOT from qwen.
# ============================================================================
MODE_FILE = REG / "qsb_wren_mode.json"
DECISIONS = REG / "qsb_wren_ross_decisions.jsonl"
ROSS_NOTES = REG / "qsb_wren_ross_notes.jsonl"

def _wren_mode():
    try:
        return json.loads(MODE_FILE.read_text()).get("mode", "ASSIST")
    except Exception:
        return "ASSIST"

def _set_mode(m):
    m = str(m).upper()
    if m not in ("OBSERVE", "ASSIST"):   # AUTOPILOT is NOT enable-able here
        return {"ok": False, "error": "only OBSERVE or ASSIST; AUTHORISED_AUTOPILOT is NOT_AUTHORISED in Stage C"}
    MODE_FILE.write_text(json.dumps({"mode": m, "ts": now_iso()}, indent=2))
    return {"ok": True, "mode": m}

def _svc_active(unit):
    try:
        import subprocess
        return subprocess.run(["systemctl", "is-active", unit], capture_output=True, text=True, timeout=4).stdout.strip()
    except Exception:
        return "unknown"

def wren_truth_state():
    """The authoritative Wren state — each component freshly probed, with source + ts.
    A dashboard HTTP 200 is labelled as PROCESS-reachable only; the mind is NOT claimed
    intelligent from a process being up."""
    ts = now_iso()
    def _up(url):
        try:
            ...
            return False
    dash_up = _up("http://127.0.0.1:8851/status") or _up("http://127.0.0.1:8851/")
    conc_up = True  # this IS the concierge answering
    watcher = _svc_active("qsb-wren-watcher")
    mind = _svc_active("qsb-wren-mind")
    comps = {
        "dashboard":  {"state": "PROCESS_REACHABLE" if dash_up else "UNREACHABLE",
                       "endpoint": "127.0.0.1:8851", "source": "DIRECT HTTP PROBE", "probe_ts": ts,
                       "note": "process reachable != mind intelligent"},
        "concierge":  {"state": "PROCESS_REACHABLE" if conc_up else "UNREACHABLE",
                       "endpoint": "0.0.0.0:8857", "source": "SELF", "probe_ts": ts},
        "watcher":    {"state": "RUNNING" if watcher == "active" else watcher.upper(),
                       "source": "systemd qsb-wren-watcher", "probe_ts": ts},
        "mind_runtime": {"state": "RUNNING" if mind == "active" else mind.upper(),
                       "source": "systemd qsb-wren-mind (+ on-demand qwen2.5:14b)", "probe_ts": ts,
                       "note": "process running; semantic response proven only when the local agent replies"},
    }
    # home-screen Wren state (never a single vague 'ONLINE')
    if not dash_up:
        home = "NEEDS ATTENTION"
    elif _wren_mode() == "OBSERVE":
        home = "WATCHING"
    else:
        home = "AT DESK"
    return {"ts": ts, "role": "ross_private_concierge", "mode": _wren_mode(),
            "home_state": home, "components": comps,
            "self_execute": False, "self_approve": False, "self_close": False,
            "authorised_autopilot": "NOT_AUTHORISED",
            "evidence_note": "component states are live probes; Wren's own LLM self-assessment is ADVISORY, not authoritative"}

def _tail_jsonl(path, n=60):
    rows = []
    if path.exists():
        for l in path.read_text(errors="ignore").splitlines()[-n:]:
            try:
...
def council_groups():
    """Read the Task Council snapshot and bucket into Stage-C groups (read-only)."""
    snap = _getj("http://127.0.0.1:8852/tasks/data") or {}
    tasks = snap.get("tasks", []) if isinstance(snap, dict) else []
    g = {k: 0 for k in ("drafts", "waiting_for_ross", "submitted", "in_progress",
                          "acknowledged", "ready_for_signoff", "returned", "rejected", "completed", "other")}
    for t in tasks:
        s = (t.get("state") or "").lower()
        if s in ("open",): g["submitted"] += 1
        elif s == "acknowledged": g["acknowledged"] += 1
        elif s == "in_progress": g["in_progress"] += 1
        elif s in ("done", "completed"): g["completed"] += 1
        elif s in ("rejected", "denied"): g["rejected"] += 1
        else: g["other"] += 1
    return {"ts": now_iso(), "total": len(tasks), "groups": g, "source": "127.0.0.1:8852/tasks/data (read-only)"}
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p == "/api/identity":
            return self._json(200, {"id": "wren_concierge", "role": "ross_private_concierge",
                                    "service": "qsb-wren-concierge-dash", "port": PORT,
                                    "mode": _wren_mode(), "authorised_autopilot": "NOT_AUTHORISED", "ts": now_iso()})
        if p == "/api/state":
            return self._json(200, wren_truth_state())
        if p == "/api/mode":
            return self._json(200, {"mode": _wren_mode(), "options": ["OBSERVE", "ASSIST"],
                                    "authorised_autopilot": "NOT_AUTHORISED", "ts": now_iso()})
        if p == "/api/task_council":
            return self._json(200, council_groups())
        if p == "/api/decisions":
            return self._json(200, {"ts": now_iso(), "decisions": _tail_jsonl(DECISIONS, 80)})
        if p == "/api/history":
            return self._json(200, {"ts": now_iso(), "notes": _tail_jsonl(ROSS_NOTES, 40),
                                    "decisions": _tail_jsonl(DECISIONS, 40),
                                    "activity": _tail_jsonl(REG / "qsb_wren_concierge_activity.jsonl", 40)})
        if p == "/api/briefing":
    ...
    def do_POST(self):
        ...
        if p == "/api/ross_note":
            note = (b.get("note") or b.get("text") or "")[:500]
            with open(ROSS_NOTES, "a") as f:
                f.write(json.dumps({"ts": now_iso(), "actor": "ross", "note": note}) + "\n")
            log_activity("ross_note", item=note[:60], actor="ross")
            return self._json(200, {"ok": True, "recorded": True, "ts": now_iso()})
        if p == "/api/mode":
            return self._json(200, _set_mode(b.get("mode", "")))
        if p == "/api/submit_task":
            # GATED: explicit Ross-approved draft id + confirmation; Task Council only;
            # NO auto-claim, NO auto-complete, NO shell. Append-only audit.
            if not b.get("ross_confirmed"):
                return self._json(400, {"ok": False, "error": "ross_confirmed=true required (final confirmation screen)"})
            draft_id = str(b.get("draft_id") or "")
            d = next((x for x in load_drafts() if str(x.get("id")) == draft_id), None)
            if not d:
                return self._json(404, {"ok": False, "error": "draft not found; only an existing Ross-approved draft may be submitted"})
            payload = {"title": (d.get("title") or "Wren-drafted task")[:120],
                       "description": ("[Ross-approved via Wren concierge] " + str(d.get("description") or d.get("request") or ""))[:1000],
                       "actor": "ross", "priority": d.get("priority", "normal")}
            try:
                req = urllib.request.Request("http://127.0.0.1:8852/tasks/create",
                                             data=json.dumps(payload).encode(), method="POST",
                                             headers={"Content-Type": "application/json"})
                res = json.loads(urllib.request.urlopen(req, timeout=6).read().decode())
            except Exception as e:
                return self._json(502, {"ok": False, "error": "council submit failed: " + str(e)[:100]})
            tid = res.get("task_id")
            with open(REG / "qsb_wren_submit_audit.jsonl", "a") as f:
                f.write(json.dumps({"ts": now_iso(), "type": "submit_task", "draft_id": draft_id,
                                    "task_id": tid, "actor": "ross", "auto_claim": False, "auto_complete": False}) + "\n")
            log_activity("submit_task", item=str(tid), result="submitted to Task Council (no auto-claim/complete)", actor="ross")
            return self._json(200, {"ok": bool(tid), "task_id": tid,
                                    "note": "submitted to Task Council; NOT claimed, NOT completed"})
        return self._json(404, {"ok": False, "error": "not found"})
