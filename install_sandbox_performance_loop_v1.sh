...
echo "======================================================"
echo "  QSB Tower V1.3 — Sandbox Performance Loop V1"
echo "======================================================"
echo "Mode: repeated sandbox ticks + paper-only performance scoring"
...
for f in \
  ...
  data/registries/sandbox_performance_policy.json \
  data/registries/sandbox_performance_latest.json
do
  [ -f "$f" ] && cp -f "$f" "data/backups/$(basename "$f").backup_before_sandbox_performance_${TS}" || true
done

echo
echo "=== WRITE PERFORMANCE POLICY ==="
cat > data/registries/sandbox_performance_policy.json <<'JSON'
{
  "policy": "sandbox_performance_loop_v1",
  "version": "1.0",
  "mode": "paper_only_performance_tracking",
  ...
  "kernel_commentary_enabled": true,
  ...
  "notes": "Runs repeated worker sandbox ticks and scores paper-only signal observations. No order endpoint is allowed."
}
JSON

echo
echo "=== CREATE SANDBOX PERFORMANCE LOOP MODULE ==="
cat > src/tower/sandbox_performance_loop.py <<'PY'
#!/usr/bin/env python3
"""
...
echo
echo "=== CREATE PERFORMANCE LOOP SCRIPTS ==="
cat > scripts/sandbox_performance_loop.sh <<'SH'
#!/usr/bin/env bash
set -euo pipefail
cd /vaults/nvme0/qsb_tower_v1
export PYTHONPATH=/vaults/nvme0/qsb_tower_v1/src
...
PY
SH
chmod +x scripts/sandbox_performance_loop.sh

cat > scripts/sandbox_performance_status.sh <<'SH'
#!/usr/bin/env bash
set -euo pipefail
cd /vaults/nvme0/qsb_tower_v1
export PYTHONPATH=/vaults/nvme0/qsb_tower_v1/src
...
PY
SH
chmod +x scripts/sandbox_performance_status.sh

cat > scripts/sandbox_performance_quick.sh <<'SH'
#!/usr/bin/env bash
set -euo pipefail
cd /vaults/nvme0/qsb_tower_v1
./scripts/sandbox_performance_loop.sh 3 5 EUR_USD,GBP_USD,USD_JPY
SH
chmod +x scripts/sandbox_performance_quick.sh

echo
echo "=== CREATE TEST ==="
cat > tests/test_sandbox_performance_loop_v1.py <<'PY'
import sys
import py_compile
from pathlib import Path
...
python3 -m py_compile src/tower/sandbox_performance_loop.py
python3 tests/test_sandbox_performance_loop_v1.py

echo
echo "=== RUN QUICK PERFORMANCE LOOP: 3 TICKS / 5 SECONDS ==="
./scripts/sandbox_performance_loop.sh 3 5 EUR_USD,GBP_USD,USD_JPY | tee data/runtime/sandbox_performance_first_run_output.json | head -220

echo
echo "=== PERFORMANCE STATUS ==="
./scripts/sandbox_performance_status.sh
...
echo
echo "======================================================"
echo "  SANDBOX PERFORMANCE LOOP V1 COMPLETE"
echo "======================================================"
echo "Run again:"
echo "  ./scripts/sandbox_performance_loop.sh 5 10 EUR_USD,GBP_USD,USD_JPY"
echo
echo "Faster check:"
echo "  ./scripts/sandbox_performance_quick.sh"
echo
