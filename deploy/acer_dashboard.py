...
PORT   = 9140
MIND   = Path.home() / "qsb" / "mind_acer.json"
CHAT   = Path.home() / "qsb" / "acer_chat.jsonl"
...
PIPELINE_PEERS = {"tp_pip": "Pip (CEO)", "bill": "Bill"}  # + self (Pip) + governor(hub)
...
def state():
    ...
    pipeline = []
    # self = Pip
    pipeline.append({"who": "Acer (CEO, this box)", "id": "acer_cass",
...
<meta name=viewport content="width=device-width,initial-scale=1"><title>ACER - Command Cathedral</title>
<style>
...
<h1><b>ACER</b> &middot; Command Cathedral</h1>
...
async function tick(){
 ...
 document.getElementById('foot').textContent='sources: 127.0.0.1:11434 (brain) - 127.0.0.1:9130 (router) - mind_acer.json - 192.168.1.23:8890 (Pi hub). No HQ, no Claude, no surrogate.';
}
...
if __name__ == "__main__":
    print("ACER dashboard (standalone) on http://127.0.0.1:%d" % PORT)
