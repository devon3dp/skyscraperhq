...
RUN_DIR="$RUN_ROOT/${STAMP}_fix_autonomous_gene_pool_dash_smoke"
REPORT="$RUN_DIR/reports/fix_autonomous_gene_pool_dash_smoke_report.txt"
...
echo "============================================================"
echo "SKYSCRAPERHQ — FIX AUTONOMOUS GENE POOL DASH + SMOKE TEST"
echo "Generated: $(date -Is)"
echo "Port: $PORT"
echo "============================================================"
echo "Rules:"
echo " - Autonomous dashboard. No manual options needed."
...
echo " - Full keys stay local and are never printed."
echo "============================================================"
...
[ -f "$PROJECT/vaults/gene_pool/gene_pool_keys.secure.env" ] && cp -a "$PROJECT/vaults/gene_pool/gene_pool_keys.secure.env" "$RUN_DIR/backups/gene_pool_keys.secure.env.bak_$STAMP"
[ -f "$PROJECT/data/registries/gene_pool_keys_public.json" ] && cp -a "$PROJECT/data/registries/gene_pool_keys_public.json" "$RUN_DIR/backups/gene_pool_keys_public.json.bak_$STAMP"

echo
echo "===== 1. WRITE IMPROVED AUTONOMOUS DASH APP ====="
...
def auto_route_once():
    ...
    if provider:
        ...
        event({
            ...
            "task": task,
            "status": "blocked",
            "detail": "no API Gene Pool key found; CEOs do not fall back to Wren/local GPU"
        })
...
def metrics():
    ...
    return {
        ...
        "rev": {
            "router": min(100, 30 + (AUTONOMY["route_count"] % 65)),
            "api_pool": min(100, active * 12),
            "ceo_load": random.randint(35, 90),
            "wren_guard": 100
        }
    }
...
<style>
:root{--panel:#071426;--line:#1e4566;--text:#e8f7ff;--muted:#8ca9bd;--cyan:#42d9ff;--green:#45f59b;--amber:#ffc857;--red:#ff5d7d}
...
header{padding:16px 20px;border-bottom:1px solid var(--line);background:rgba(0,0,0,.35);position:sticky;top:0;z-index:10;backdrop-filter:blur(10px)}
...
.statusbox{background:#061426;border:1px solid var(--line);border-radius:14px;padding:10px}.big{font-size:26px;font-weight:900}
...
.prov.ok{border-color:rgba(69,245,155,.75);box-shadow:0 0 14px rgba(69,245,155,.08)}.prov.bad{border-color:rgba(255,93,125,.55)}
...
.pulse{display:inline-block;width:10px;height:10px;border-radius:50%;background:var(--green);box-shadow:0 0 16px var(--green);animation:pulse 1s infinite}@keyframes pulse{50%{opacity:.25;transform:scale(.65)}}
...
<div class="sub">No buttons. Linux/vault scan and CEO routing run automatically. Wren/GPU protected. CEOs use API Gene Pool only.</div>
</header>
...
<script>
const $=q=>document.querySelector(q);let lastSeen=0;
...
function drawWires(){const svg=$("#wires");svg.innerHTML="";const router=centre($('[data-node="Brain Router"]'));[...document.querySelectorAll(".claude,.ceo2,.ceo3,.provider")].forEach(n=>{const c=centre(n);const l=document.createElementNS("http://www.w3.org/2000/svg","line");l.setAttribute("x1",c.x);l.setAttribute("y1",c.y);l.setAttribute("x2",router.x);l.setAttribute("y2",router.y);l.setAttribute("class","line");svg.appendChild(l);});const w=centre($('[data-node="Wren"]'));const l=document.createElementNS("http://www.w3.org/2000/svg","line");l.setAttribute("x1",w.x);l.setAttribute("y1",w.y);l.setAttribute("x2",router.x);l.setAttribute("y2",router.y);l.setAttribute("class","line wrenline");svg.appendChild(l);}
function packet(aSel,bSel,color){const aEl=$(aSel),bEl=$(bSel);if(!aEl||!bEl)return;const a=centre(aEl),b=centre(bEl);const p=document.createElement("div");p.className="packet";if(color){p.style.background=color;p.style.boxShadow=`0 0 18px ${color}`;}p.style.setProperty("--x1",(a.x-5)+"px");p.style.setProperty("--y1",(a.y-5)+"px");p.style.setProperty("--x2",(b.x-5)+"px");p.style.setProperty("--y2",(b.y-5)+"px");$("#flow").appendChild(p);setTimeout(()=>p.classList.add("go"),20);setTimeout(()=>p.remove(),1500);}
...
function renderProviders(ps){$("#providers").innerHTML=Object.values(ps||{}).map(p=>{const ok=(p.key_count||0)>0;return `<div class="prov ${ok?'ok':'bad'}"><b>${p.label}</b><small>${p.role}</small><br><small>keys: ${p.key_count||0} · ${p.tier}</small><br><small>${(p.keys||[]).slice(0,1).map(k=>k.masked+" · "+k.fingerprint).join("")}</small></div>`;}).join("");}
function renderEvents(logs){$("#stream").innerHTML=(logs||[]).slice(-80).reverse().map(e=>{const cls=e.status==="blocked"||e.status==="error"?"bad":e.status==="selected"||e.status==="stored"?"ok":"warn";return `<div class="event"><span class="${cls}">●</span> ${e.ts||""}<br>${e.from||"?"} → ${e.to||"?"} · ${e.provider||""} · ${e.task||""}<br><small>${e.detail||""}</small></div>`;}).join("");}
function animate(e){if(!e)return;if(e.event==="request")packet(ceoSel(e.from),'[data-node="Brain Router"]');if(e.event==="dispatch"&&e.provider&&e.provider!=="none")packet('[data-node="Brain Router"]',provSel(e.provider),'#42d9ff');if(e.event==="return"&&e.provider&&e.provider!=="none"){packet(provSel(e.provider),'[data-node="Brain Router"]','#45f59b');setTimeout(()=>packet('[data-node="Brain Router"]',ceoSel(e.to),'#45f59b'),500);}if(e.event==="auto_scan")packet('[data-node="Wren"]','[data-node="Brain Router"]','#45f59b');}
async function live(){const d=await getJSON("/api/live");const m=d.metrics||{},rev=m.rev||{};setGauge("#g_router",rev.router);setGauge("#g_pool",rev.api_pool);setGauge("#g_ceo",rev.ceo_load);setGauge("#g_wren",rev.wren_guard);$("#keycount").textContent=m.stored_key_count||0;$("#providerscount").textContent=m.active_provider_count||0;$("#routecount").textContent=(m.autonomy&&m.autonomy.route_count)||0;renderProviders(d.providers||{});renderEvents(d.logs||[]);$("#statebox").textContent=JSON.stringify(m.autonomy||{},null,2);const logs=d.logs||[];if(logs.length>lastSeen){logs.slice(lastSeen).forEach((e,i)=>setTimeout(()=>animate(e),i*220));lastSeen=logs.length;}}
drawWires();window.addEventListener("resize",drawWires);live();setInterval(live,1300);
</script>
</body>
</html>'''
...
for f in \\
  ...
  "$PROJECT/vaults/keys/moonshot_api.env" \\
  "$PROJECT/vaults/keys/grok_api.env" \\
  "$PROJECT/vaults/keys/xai_api.env" \\
...
echo
echo "===== 6. FORCE RESCAN NOW ====="
curl -sS --max-time 240 "http://127.0.0.1:$PORT/api/rescan" > "$RUN_DIR/reports/rescan.json" || true
python3 - <<PYRESCAN
import json, pathlib
p=pathlib.Path("$RUN_DIR/reports/rescan.json")
try:
    d=json.loads(p.read_text())
    print("rescan_ok:", d.get("ok"))
    m=d.get("metrics",{})
    print("stored_key_count:", m.get("stored_key_count"))
    print("active_provider_count:", m.get("active_provider_count"))
except Exception as e:
    print("rescan parse failed:", e)
    print(p.read_text(errors="ignore")[:1200] if p.exists() else "missing")
PYRESCAN

echo
echo "===== 7. LET AUTONOMY FLOW 15 SECONDS ====="
sleep 15

echo
echo "===== 8. SMOKE TEST HEALTH ====="
curl -sS --max-time 10 "http://127.0.0.1:$PORT/health" | python3 -m json.tool || true

echo
echo "===== 9. SMOKE TEST LIVE STATE ====="
...
if p.exists():
    ...
    print("missing public store")
PYPUB

echo
echo "===== 12. OPEN DASHBOARD ====="
...
echo
echo "============================================================"
echo "DONE — IMPROVED AUTONOMOUS DASH RUNNING"
echo "Open:"
echo "$LOCAL"
echo "$LAN"
echo
echo "Secure key store, do not send:"
...
cp -a "$RUN_DIR/reports/rescan.json" "$SEND/rescan.json"
