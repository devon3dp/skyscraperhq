...
const P = {
  ...
  tp:   {title:'TP-Pip',     sub:'192.168.1.74:8871',brain:'local llama3.2 + gene', proxy:'/proxy/tp',   direct:()=>'http://'+location.hostname+':8852/proxy/tp'},
  acer: {title:'Acer-Cass',  sub:'192.168.1.41:8872',brain:'gene:deepseek + local', proxy:'/proxy/acer', direct:()=>'http://'+location.hostname+':8852/proxy/acer'},
};
...
async function poll(){
  let d;
  try { d = await (await fetch('/quad_monitor/health',{cache:'no-store'})).json(); }
  catch(e){ for(const k of ORDER){ setDown(k,'health probe failed: '+e); } return; }
  for (const k of ORDER){
    const r = (d.panels||{})[k];
    if (!r){ setDown(k,'no health data'); continue; }
    ...
    if(rdot) rdot.className = r.up ? 'dot up' : 'dot down';
    if(rst) rst.textContent = r.up ? ('· '+r.code) : '· DOWN';
    if (r.up){
      dot.className='dot up';
      st.textContent='live · HTTP '+r.code;
      err.classList.remove('show');
    } else {
      dot.className='dot down';
      st.textContent='DOWN · '+(r.detail||('HTTP '+r.code));
      document.getElementById('errc-'+k).textContent=(r.url||'')+'  —  '+(r.detail||('HTTP '+r.code));
