...
<div class="ctrlwrap">
 ...
 <div class="ctrlPanel">
  ...
  <div class="ctrlRow">
   ...
   <button class="btn cbtn" onclick="startCommentary()">▶️ Start live commentary</button>
  ...
  <div class="sub" style="font-size:10px;color:var(--wait)">Browser speech input is optional — may need Chromium mic permission. If unsupported it stays disabled (NEEDS WIRING).</div>
  ...
  <div class="ctrlHead" style="margin-top:8px">🎛️ Voice settings</div>
  ...
  <div class="ctrlRow" style="align-items:center;font-size:12px;color:var(--dim)">
   ...
   <label style="flex:1;min-width:190px"><input id="ovg" type="checkbox" onchange="vset('overlapGuard',this.checked)"> Prevent overlapping speech (ON)</label>
   <label style="flex:1;min-width:190px"><input id="pwh" type="checkbox" onchange="vset('pauseWhenHidden',this.checked)"> Pause speech when tab hidden (ON)</label>
  </div>
  <div class="ctrlRow">
   <button class="btn cbtn" onclick="speakTest()">🧪 Test voice</button>
   <button class="btn cbtn" onclick="speakStatus()">🔊 Speak current status once</button>
   <button class="btn cbtn red" onclick="stopAll()">⛔ Stop all speech</button>
   <button class="btn cbtn" onclick="resetVoice()">↺ Reset voice settings</button>
   <button class="btn cbtn" onclick="voiceDebug()">🐞 Show voice debug</button>
  </div>
  ...
  <div class="sub" style="font-size:11px;margin-top:8px;color:var(--stale)">🔉 Voice is local browser speech. If it sounds doubled, close duplicate Receptionist tabs or press “Stop all speech”.</div>
...
async function ck(a,id){const r=['done','clear','snooze','signoff'].includes(a)?a:'update';await post('/api/checklist/'+r,{id:id,action:a});toast(a.replace('_',' ')+' ✓');loadChecklist();}
...
function setAvatar(s){const a=$('#avatar');if(a)a.className='rcAvatar '+s;const st=$('#avState');if(st)st.textContent=s==='speaking'?'Speaking…':s==='listening'?'Listening…':muted?'Muted':'At desk';}
...
function speak(text,opts){opts=opts||{};if(!text||muted||!TTS)return;
 if(vcfg.overlapGuard){ if(isSpeaking&&!opts.interrupt)return; if(otherTabSpeaking&&!opts.interrupt)return; }
 ...
 try{speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(text);const v=pickVoice();if(v)u.voice=v;
  u.rate=+vcfg.rate||0.9;u.pitch=+vcfg.pitch||1.0;u.volume=(vcfg.volume==null?1.0:+vcfg.volume);
  ...
  u.onend=()=>{isSpeaking=false;setAvatar('idle');voiceDebug();};
...
function speakTest(){speak('Hello, this is the Skyscraper H Q receptionist, speaking with one clear voice.',{interrupt:true});}
function commStep(){if(!commLines.length)return;if(isSpeaking||otherTabSpeaking)return;if(Date.now()-lastSpeakAt<COOLDOWN)return;if(vcfg.pauseWhenHidden&&document.visibilityState==='hidden')return;const line=commLines[commIdx%commLines.length];const cl=$('#commentaryLine');if(cl)cl.textContent=line;speak(line);commIdx++;}
