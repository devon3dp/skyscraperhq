...
# ============================================================================
_P1_PHYS = {"tp": "http://192.168.1.74:8871", "acer": "http://192.168.1.41:8872"}
...
def _p1_http(key, url, path="", timeout=3, method="GET"):
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=timeout) as r:
            ...
            out["body_sample"] = body[:220].decode("utf-8", "replace")
...
def p1_truth_model():
    ...
    for k, name in [("tp", "TP-Pip"), ("acer", "Acer-Cass")]:
        ...
        surr_id = ""
        import re as _re
        _m = _re.search(r'"id"\s*:\s*"([^"]+)"', surr.get("body_sample", "") or "")
        if _m:
            surr_id = _m.group(1)
        out["ceos"][k] = {
            "name": name,
            "physical": {"endpoint": _P1_PHYS[k] + "  (/health,/whoami)",
                         **{kk: phys[kk] for kk in ("state", "code", "detail", "probe_ts", "last_ok", "last_fail")}},
            "surrogate": {"endpoint": _P1_SURR[k], "identity": surr_id,
                          **{kk: surr[kk] for kk in ("state", "code", "detail", "probe_ts", "last_ok", "last_fail")}},
            "operating_mode": _p1_mode(phys, surr),
            "evidence": "fresh direct HTTP probe of BOTH the physical box AND the HQ surrogate",
        }
...
function renderTruth(d){
  ...
  for(const k of ['acer','tp']){
    const c=d.ceos&&d.ceos[k]; if(!c) continue; const p=c.physical, s=c.surrogate;
    html+=card(c.name, c.operating_mode,
      '<div>PHYSICAL WORKER: '+_b(p.state)+' <span style="color:#64748b">'+p.endpoint+'</span><br><span style="color:#64748b">'+_tsline(p)+(p.detail?(' · '+p.detail):'')+'</span></div>'+
      '<div style="margin-top:4px">HQ SURROGATE: '+_b(s.state)+' <span style="color:#64748b">'+s.endpoint+(s.identity?(' (id:'+s.identity+')'):'')+'</span><br><span style="color:#64748b">'+_tsline(s)+'</span></div>'+
      '<div style="margin-top:4px;color:#94a3b8">Evidence: '+c.evidence+'</div>');
  }
...
async function poll(){
  ...
  for (const k of ORDER){
    ...
    if(k==='tp'||k==='acer'){
      const phys=c.physical.state, surr=c.surrogate.state; ifaceUp=(surr==='LIVE'); ep=c.surrogate.endpoint;
      if(phys==='LIVE'){ dotcls='dot up'; label='PHYSICAL LIVE · HTTP '+c.physical.code; }
      else if(surr==='LIVE'){ dotcls='dot wait'; label='⚠ HQ SURROGATE ONLY · physical '+phys; }
      else { dotcls='dot down'; label='OFFLINE · phys '+phys+' · surr '+surr; }
    } else if(k==='hq'){
