...
<div class="ctrlwrap">
 <div class="avatarCard">
  ...
  <div style="flex:1;min-width:180px">
   ...
   <div class="sub" style="font-size:10px;color:var(--wait)">idle avatar bob = COSMETIC · speaking glow only while TTS speaks</div>
  </div>
 </div>
 <div class="ctrlPanel">
...
let vcfg=Object.assign({voice:'',rate:0.9,pitch:1.0,volume:1.0,commentaryEnabled:false,overlapGuard:true,pauseWhenHidden:true,intervalMs:8000},(()=>{try{return JSON.parse(localStorage.getItem(VKEY)||'{}')}catch(e){return{}}})());
...
async function startCommentary(){await loadCommentary();if(commTimer)clearInterval(commTimer);commIdx=0;vcfg.commentaryEnabled=true;saveVcfg();commStep();commTimer=setInterval(commStep,+vcfg.intervalMs||8000);toast('live commentary on');}
function stopCommentary(){if(commTimer){clearInterval(commTimer);commTimer=null;}vcfg.commentaryEnabled=false;saveVcfg();stopAll();toast('commentary stopped');}
function toggleMute(){muted=!muted;const b=$('#muteBtn');if(b)b.textContent=muted?'🔈 Unmute':'🔇 Mute';if(muted)stopAll();setAvatar('idle');toast(muted?'muted':'unmuted');}
...
renderTabs();go('desk');populateVoices();applyVoiceUI();voiceDebug();setInterval(()=>{if(cur==='desk'||cur==='emergency')refreshDesk()},5000);setInterval(loadReports,30000);
</script></body></html>"""
