...
<script>(function(){
 ...
 function render(tc){const items=lastData.items,cards=lastData.cards;
  ...
  let done=items.filter(i=>['approved','signed_off','denied','rejected','snoozed'].includes(i.status)).concat(cards.filter(c=>['signed_off','rejected'].includes(c.status)).map(c=>({title:c.job_id+' — '+(c.interpreted_request||'').slice(0,50),status:c.status,signoff_type:c.signoff_type,proof:c.proof,ross_decision:c.ross_signoff?'signed off':''})));
