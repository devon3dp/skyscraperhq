...
COMMS_DEFS = [
    {"channel": "Telegram", "proc": "qsb_telegram_receptionist.py", "log": "qsb_telegram_audit.jsonl",
     "file": "tools/qsb_telegram_receptionist.py"},
    {"channel": "WhatsApp", "proc": "qsb_wa_inbound.js", "log": "qsb_whatsapp_sends.jsonl",
     "file": "tools/whatsapp_inbound/qsb_wa_inbound.js"},
    {"channel": "Gmail", "proc": "qsb_email_receiver.py", "log": "qsb_email_triage_log.jsonl",
     "file": "tools/qsb_email_sender.py"},
    {"channel": "Voice / phone (Twilio)", "proc": "qsb_twilio_voice_receptionist.py", "log": None,
     "file": "tools/qsb_twilio_voice_receptionist.py"},
]
...
const TABS=[['today','📋 Today'],['desk','🛎️ Desk'],['floors','🏢 Floors'],['comms','📡 Comms'],['checklist','☑️ Checklist'],['issues','⚠️ Issues'],['approvals','✅ Approvals'],['activity','📜 Activity'],['drafts','📝 Drafts'],['brief','🌅 Brief'],['inbox','📥 Inbox'],['handover','🔀 Handover'],['emergency','🚨 Emergency']];
...
async function loadComms(){const d=await j('/api/comms');$('#comms').innerHTML=d.channels.map(c=>`<div class="node ${cls(c.status)}"><span class="stat">${c.status}</span><b><span class="dot"></span>${c.channel}</b>
 ...
 <div class="detail">last event: ${c.last_event||'—'} · log rows: ${c.log_rows??'—'}</div>
 <div class="detail">send: <b style="color:var(--off)">${c.send_enabled?'YES':'NO'}</b> · read bodies: <b style="color:var(--off)">${c.read_body_enabled?'YES':'NO'}</b> · needs approval: <b style="color:var(--gold)">${c.needs_approval?'YES':'NO'}</b></div></div>`).join('')}
async function load(){if(cur==='desk'){refreshDesk();loadLinks()}if(cur==='today')loadToday();if(cur==='floors')loadFloors();if(cur==='comms')loadComms();if(cur==='checklist')loadChecklist();if(cur==='activity')loadActivity();if(cur==='drafts')loadDrafts();if(cur==='brief')loadBrief();if(cur==='approvals')loadApprovals();if(cur==='issues')loadIssues();if(cur==='inbox')loadInbox();if(cur==='handover')loadHandover();if(cur==='emergency')refreshDesk();loadReports()}
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p == "/api/receptionist_drive":
            return self._json(200, {"ts": now_iso(), "status": "NOT SCANNED",
                                    "note": "drive index not built — read-only drive discovery needs separate approval"})
        if p == "/api/links":
            ip = HQ_IP
