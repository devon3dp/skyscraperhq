...
<script>(function(){
 ...
 let rec=null,listening=false,filter='active',showDone=false,sel={},dec={},lastData={items:[],cards:[]};
 ...
 function doneCard(it){const col=(it.status=='signed_off'||it.status=='approved')?'#31d07f':(it.status=='denied'||it.status=='rejected')?'#ff5d5d':'#f5b942';return `<div style="background:#0d1420;border:1px solid #22334a;border-radius:8px;padding:6px 10px;margin-bottom:5px;font-size:12px"><span style="font-size:10px;font-weight:800;padding:1px 6px;border:1px solid ${col};border-radius:5px;color:${col}">${(it.status||'').toUpperCase()}</span> ${it.title}${it.ross_decision?(' <span style="color:#7d8ea3">· Ross: '+it.ross_decision+'</span>'):''}</div>`}
 async function load(){const d=await j('/api/approval_checklist');const jc=await j('/api/job_cards');const tc=await j('/api/task_council_bridge');
  lastData={items:(d&&d.items)||[],cards:(jc&&jc.cards)||[]};render(tc)}
 function render(tc){const items=lastData.items,cards=lastData.cards;
  ...
  let done=items.filter(i=>['approved','signed_off','denied','rejected','snoozed'].includes(i.status)).concat(cards.filter(c=>['signed_off','rejected'].includes(c.status)).map(c=>({title:c.job_id,status:c.status,ross_decision:c.ross_signoff?'signed off':''})));
  ...
  g('rarSignoff').innerHTML=signoff.length?signoff.map(c=>`<div style="background:#0f1a2a;border:1px solid #31d07f;border-radius:10px;padding:9px 11px;margin-bottom:7px">
    ...
    <div style="display:flex;gap:5px;flex-wrap:wrap;margin-top:6px">
     <button class="ccb g" style="padding:6px 11px;font-size:12px" onclick="jcSign('${c.job_id}','signoff')">✔ Sign off</button>
