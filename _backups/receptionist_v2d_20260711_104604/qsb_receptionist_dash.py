...
// V2C voice / avatar / commentary (browser speechSynthesis only — no cloud, no secrets, no sending)
let muted=false,commTimer=null,commLines=[],commIdx=0,recog=null,listening=false;
const TTS=('speechSynthesis' in window);
function setAvatar(s){const a=$('#avatar');if(!a)return;a.className='rcAvatar '+s;const st=$('#avState');if(st)st.textContent=s==='speaking'?'Speaking…':s==='listening'?'Listening…':muted?'Muted':'At desk';}
function speak(t){if(!t||muted||!TTS)return;try{const u=new SpeechSynthesisUtterance(t);u.rate=1;u.onstart=()=>setAvatar('speaking');u.onend=()=>setAvatar('idle');speechSynthesis.cancel();speechSynthesis.speak(u);}catch(e){}}
...
async function speakStatus(){await loadCommentary();speak(commLines[0]);}
async function speakIssue(){const d=await j('/api/issues');const i=d.issues[0];speak(i&&i.kind!=='none'?(i.sev+' issue: '+i.kind+'. '+i.detail):'No issues detected.');}
async function speakApprovals(){const d=await j('/api/wiring_approvals');speak(d.queue.length+' wiring approvals waiting. '+d.queue.slice(0,3).map(a=>a.title).join('. '));}
async function speakReport(){const d=await j('/api/latest_reports');const r=(d.reports||[])[0];speak(r?('Latest report: '+r.name+'. '+(r.verdict||'')):'No reports.');}
async function startCommentary(){await loadCommentary();if(commTimer)clearInterval(commTimer);commIdx=0;const step=()=>{if(!commLines.length)return;const line=commLines[commIdx%commLines.length];const cl=$('#commentaryLine');if(cl)cl.textContent=line;speak(line);commIdx++;};step();commTimer=setInterval(step,6000);toast('live commentary on');}
function stopCommentary(){if(commTimer){clearInterval(commTimer);commTimer=null;}if(TTS)speechSynthesis.cancel();setAvatar('idle');toast('commentary stopped');}
function toggleMute(){muted=!muted;const b=$('#muteBtn');if(b)b.textContent=muted?'🔈 Unmute':'🔇 Mute';if(muted&&TTS)speechSynthesis.cancel();setAvatar('idle');toast(muted?'muted':'unmuted');}
...
async function signoffLatest(){const d=await j('/api/checklist');const it=(d.active||[]).find(i=>i.status==='open'||i.status==='needs_approval');if(!it)return toast('no item to sign off');await post('/api/checklist/signoff',{id:it.id});toast('signed off: '+it.title);speak('Signed off: '+it.title);if(cur==='checklist')loadChecklist();}
